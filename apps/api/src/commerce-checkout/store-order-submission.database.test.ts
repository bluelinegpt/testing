...
describe.skipIf(!runDatabaseTests)(
  "StoreOrderSubmissionService (Customer Commerce Prompt C3)",
  () => {
    it("revalidates, creates exactly one Store Order per idempotency key, and snapshots correctly", async () => {
      ...
      try {
        await database.transaction().execute(async (transaction) => {
          ...
          const dubai = await sql<{
            id: string;
          }>`select id from emirates where name_en = 'Dubai'`.execute(transaction);
          ...
          values(${ids.commerceId}::uuid,'C3 Shop','delivery_company_registered','approved')`.execute(
            transaction,
          );
          ...
          await sql`insert into trader_storefronts(
            ...
            'C3 Shop',${`c3-shop-${short}`},'general','modern','published',now())`.execute(
            transaction,
          );
          ...
          expect(created.items[0]!.selectedOptionsSnapshot).toEqual([
            { group: "Size", value: "Medium" },
          ]);
        ...
        `.execute(transaction);
          ...
          const zeroCompanyRow = await sql<{
