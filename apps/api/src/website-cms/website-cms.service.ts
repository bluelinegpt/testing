...
export function renderLocalizedSitemap(origin: string, entries: LocalizedSitemapEntry[]) {
  ...
  const urls = entries.map(({ path, lastmod, alternates, xDefault }) => {
    ...
    const links = [
      ...alternates.map(
        (alternate) =>
          `<xhtml:link rel="alternate" hreflang="${escape(alternate.locale)}" href="${escape(`${origin}${alternate.path}`)}" />`,
      ),
      ...(xDefault
        ? [
            `<xhtml:link rel="alternate" hreflang="x-default" href="${escape(`${origin}${xDefault}`)}" />`,
          ]
        : []),
    ].join("");
...
function pagePayload(input: WebsitePageContentDto) {
  return {
    ...
    seo: {
      ...
      canonical: input.canonicalPath
        ? requireSafePath(input.canonicalPath)
        : `/${input.pageKey === "home" ? "" : input.pageKey}`,
      robotsIndex: input.robotsIndex,
      robotsFollow: input.robotsFollow,
      ogImage: input.ogImage ?? null,
    },
...
function planPayload(input: PricingPlanDto) {
  ...
  if (maxOrders !== null && maxOrders < input.minOrders)
    throw new BadRequestException("Plan max orders cannot be lower than min orders.");
  return {
...
export function isImage(
  bytes: Uint8Array,
  declared: string,
): { ok: true; mediaType: string; ext: string } | { ok: false; reason: string } {
  ...
  if (
    ...
    !(detected.mediaType === "image/jpeg" && normalized === "image/jpg")
  )
...
export function imageDimensions(
  ...
  if (mediaType === "image/png" && b.length >= 24)
    return { width: b.readUInt32BE(16), height: b.readUInt32BE(20) };
  if (mediaType === "image/webp" && b.length >= 30 && b.toString("ascii", 12, 16) === "VP8X")
...
export function isWebsiteMedia(
  bytes: Uint8Array,
  declared: string,
): { ok: true; mediaType: string; ext: string } | { ok: false; reason: string } {
  ...
  if (declared.toLowerCase() !== "video/mp4")
    return { ok: false, reason: "declared_media_type_mismatch" };
...
function websiteMediaValidationMessage(reason: string) {
  const help: Record<string, string> = {
    declared_media_type_mismatch:
      "The selected file's browser-reported type does not match its real image data. Export it again as JPG, PNG, or WebP instead of renaming the file.",
    empty_file: "The selected file is empty. Please choose the original image file again.",
    file_too_large:
      "The selected file is larger than the 20 MB website media limit. Please compress or export a smaller JPG, PNG, WebP, or MP4.",
    markup_or_script_rejected:
      "SVG, HTML, XML, and script-like files are not accepted for blog media. Please export the image as JPG, PNG, or WebP.",
    unsupported_media_signature:
      "The selected file is not a readable JPG, PNG, WebP, or MP4 file. HEIC, AVIF, GIF, SVG, PDF, and renamed files are not supported.",
  };
  return (
    help[reason] ??
    "The selected file could not be validated as JPG, PNG, WebP, or MP4. Please export it again and retry."
  );
}
...
export class WebsiteCmsService {
  ...
  public async publicBundle(locale = "en") {
    ...
    const [pages, pricing, features, faqs, navigation, contact] = await Promise.all([
      sql<any>`select page_key as "pageKey", locale, visible, published_content as content from platform_website_pages where status='published' and published_content is not null and locale in (${safeLocale}, 'en') order by case when locale=${safeLocale} then 0 else 1 end`.execute(
        this.db,
      ),
      sql<any>`select plan_key as "planKey", locale, published_data as data, active, sort_order as "sortOrder" from platform_website_pricing_plans where status='published' and active and published_data is not null and locale in (${safeLocale}, 'en') order by sort_order`.execute(
        this.db,
      ),
      sql<any>`select slug, locale, published_data as data, audience, category, feature_status as "featureStatus", visible, sort_order as "sortOrder" from platform_website_features where status='published' and visible and published_data is not null and locale in (${safeLocale}, 'en') order by sort_order`.execute(
        this.db,
      ),
      sql<any>`select faq_key as "faqKey", locale, published_data as data, audience, category, visible, sort_order as "sortOrder" from platform_website_faqs where status='published' and visible and published_data is not null and locale in (${safeLocale}, 'en') order by sort_order`.execute(
        this.db,
      ),
      sql<any>`select item_key as "itemKey", locale, label, destination, visible, sort_order as "sortOrder" from platform_website_navigation_items where visible and locale in (${safeLocale}, 'en') order by sort_order`.execute(
        this.db,
      ),
      sql<any>`select published_data as data from platform_website_contact_settings where id=true`.execute(
        this.db,
      ),
    ]);
    const prefer = (rows: any[], key: string) =>
      Object.values(
        rows.reduce(
          (acc, row) => {
            acc[row[key]] =
              acc[row[key]] && acc[row[key]].locale === safeLocale ? acc[row[key]] : row;
            return acc;
          },
  ...
  public async sitemapEntries() {
    ...
    for (const row of rows) {
      ...
      if (!path.startsWith("/") || path.includes("?") || path.includes("#") || excluded.test(path))
        continue;
    ...
    const entries = [...groups.values()].flatMap((group) =>
      group.map((row) => ({
        ...
        alternates: group.map((alternate) => ({
          locale: alternate.locale,
          path: localizedPath(alternate),
        })),
        xDefault: group.find((alternate) => alternate.locale === "en")
          ? localizedPath(group.find((alternate) => alternate.locale === "en")!)
          : null,
      })),
    );
    return [...new Map(entries.map((entry) => [entry.path, entry])).values()].sort((a, b) =>
  ...
  public async sitemapXml() {
    const settings = (
      await sql<{ canonical_base_url: string }>`
  ...
  public async adminBundle() {
    const [
      ...
      sql<any>`select * from platform_website_pricing_plans order by sort_order, plan_key, locale`.execute(
        this.db,
      ),
      sql<any>`select * from platform_website_features order by sort_order, slug, locale`.execute(
        this.db,
      ),
      sql<any>`select * from platform_website_faqs order by sort_order, faq_key, locale`.execute(
        this.db,
      ),
      sql<any>`select * from platform_help_categories order by sort_order, slug, locale`.execute(
        this.db,
      ),
      sql<any>`select a.*, c.slug as category_slug, c.name as category_name from platform_help_articles a left join platform_help_categories c on c.id=a.category_id order by a.sort_order, a.slug, a.locale`.execute(
        this.db,
      ),
      sql<any>`select id, public_url as "publicUrl", original_filename as "originalFilename", media_type as "mediaType", size_bytes as "sizeBytes", width, height, alt_text as "altText", caption, created_at as "createdAt" from platform_website_media where deleted_at is null order by created_at desc limit 50`.execute(
        this.db,
      ),
      sql<any>`select * from platform_website_navigation_items order by sort_order, item_key, locale`.execute(
        this.db,
      ),
      sql<any>`select * from platform_website_contact_settings where id=true`.execute(this.db),
      sql<any>`select entity_type as "entityType", entity_key as "entityKey", locale, event_type as "eventType", created_at as "createdAt" from platform_website_revisions order by created_at desc limit 30`.execute(
        this.db,
      ),
    ]);
    return {
  ...
  public async helpHome(locale = "en") {
    ...
    return {
      locale: safeLocale,
      direction: safeLocale === "ar" ? "rtl" : "ltr",
      categories: prefer(categories, "slug"),
      articles: prefer(articles, "slug"),
    };
  }
  ...
  public async saveHelpArticle(input: HelpArticleDto, actor: string) {
    const category = (
      await sql<{
        id: string;
      }>`select id from platform_help_categories where slug=${input.categorySlug} and locale=${input.locale}`.execute(
        this.db,
      )
    ).rows[0];
  ...
  public async publishHelpArticle(slug: string, locale: string, actor: string) {
    const row = (
      await sql<any>`update platform_help_articles set status='published',published_by_account_id=${actor}::uuid,published_at=coalesce(published_at,now()),updated_at=now() where slug=${slug} and locale=${locale} returning *`.execute(
        this.db,
      )
    ).rows[0];
  ...
  public async archiveHelpArticle(slug: string, locale: string, actor: string) {
    const row = (
      await sql<any>`update platform_help_articles set status='archived',updated_by_account_id=${actor}::uuid,updated_at=now() where slug=${slug} and locale=${locale} returning *`.execute(
        this.db,
      )
    ).rows[0];
    if (!row) throw new NotFoundException("help_article_not_found");
  ...
  private async revision(
    ...
    await sql`insert into platform_website_revisions(entity_type,entity_key,locale,event_type,actor_account_id,snapshot) values(${entityType},${entityKey},${locale},${eventType},${actor}::uuid,${JSON.stringify(snapshot)}::jsonb)`.execute(
      this.db,
    );
  }
  ...
  public async publishPage(pageKey: string, locale: string, actor: string) {
    const row = (
      await sql<any>`update platform_website_pages set published_content=draft_content,status='published',published_by_account_id=${actor}::uuid,published_at=now(),updated_at=now() where page_key=${pageKey} and locale=${locale} returning *`.execute(
        this.db,
      )
    ).rows[0];
  ...
  public async publishPricing(planKey: string, locale: string, actor: string) {
    const row = await this.db.transaction().execute(async (trx) => {
      const published = (
        await sql<any>`update platform_website_pricing_plans set published_data=draft_data,status='published',published_by_account_id=${actor}::uuid,published_at=now(),updated_at=now() where plan_key=${planKey} and locale=${locale} returning *`.execute(
          trx,
        )
      ).rows[0];
      if (!published) throw new NotFoundException("pricing_plan_not_found");
      const active = (
        await sql<any>`select plan_key,published_data as data,active from platform_website_pricing_plans where locale=${locale} and status='published' and active and published_data is not null order by sort_order`.execute(
          trx,
        )
      ).rows;
      this.validatePricingRanges(
        active.map((item) => ({ key: item.plan_key, ...(item.data as Record<string, unknown>) })),
      );
      await sql`insert into platform_website_revisions(entity_type,entity_key,locale,event_type,actor_account_id,snapshot) values('pricing',${planKey},${locale},'published',${actor}::uuid,${JSON.stringify(published.published_data)}::jsonb)`.execute(
        trx,
      );
      return published;
    });
  ...
  private validatePricingRanges(plans: Array<Record<string, unknown>>) {
    ...
    const ranges = plans
      .map((plan) => {
        ...
        if (names.has(name))
          throw new ConflictException("Duplicate active published pricing plan name.");
        names.add(name);
        const price = Number(plan.price);
        const min = Number(plan.minOrders);
        const max =
          plan.maxOrders === null || plan.maxOrders === undefined
            ? Number.POSITIVE_INFINITY
            : Number(plan.maxOrders);
        if (
          Number.isNaN(price) ||
          price < 0 ||
          Number.isNaN(min) ||
          ...
          throw new BadRequestException("Invalid pricing range or price.");
        return { name, min, max };
      })
      .sort((a, b) => a.min - b.min);
    for (let i = 1; i < ranges.length; i += 1) {
      if (ranges[i]!.min <= ranges[i - 1]!.max)
        throw new BadRequestException("Pricing ranges overlap.");
    }
  }
  ...
  public async publishFeature(slug: string, locale: string, actor: string) {
    const row = (
      await sql<any>`update platform_website_features set published_data=draft_data,status='published',published_by_account_id=${actor}::uuid,published_at=now(),updated_at=now() where slug=${slug} and locale=${locale} returning *`.execute(
        this.db,
      )
    ).rows[0];
  ...
  public async publishFaq(faqKey: string, locale: string, actor: string) {
    const row = (
      await sql<any>`update platform_website_faqs set published_data=draft_data,status='published',published_by_account_id=${actor}::uuid,published_at=now(),updated_at=now() where faq_key=${faqKey} and locale=${locale} returning *`.execute(
        this.db,
      )
    ).rows[0];
  ...
  public async saveContactDraft(input: WebsiteContactSettingsDto, actor: string) {
    ...
    const row = (
      await sql<any>`update platform_website_contact_settings set draft_data=${JSON.stringify(data)}::jsonb,status='draft',updated_by_account_id=${actor}::uuid,updated_at=now() where id=true returning *`.execute(
        this.db,
      )
    ).rows[0];
  ...
  public async publishContact(actor: string) {
    const row = (
      await sql<any>`update platform_website_contact_settings set published_data=draft_data,status='published',published_by_account_id=${actor}::uuid,published_at=now(),updated_at=now() where id=true returning *`.execute(
        this.db,
      )
    ).rows[0];
  ...
  public async saveNavigation(input: NavigationItemDto, actor: string) {
    ...
    const row = (
      await sql<any>`insert into platform_website_navigation_items(item_key,locale,label,destination,visible,sort_order,updated_by_account_id) values(${input.itemKey},${input.locale},${cleanText(input.label)},${input.destination},${input.visible},${input.sortOrder},${actor}::uuid) on conflict(item_key,locale) do update set label=excluded.label,destination=excluded.destination,visible=excluded.visible,sort_order=excluded.sort_order,updated_by_account_id=excluded.updated_by_account_id,updated_at=now() returning *`.execute(
        this.db,
      )
    ).rows[0];
    await this.revision(actor, "navigation", input.itemKey, input.locale, "saved", {
      label: input.label,
  ...
  public async uploadMedia(file: UploadedFile | undefined, body: MediaAltDto, actor: string) {
    ...
    if (!validation.ok)
      throw new BadRequestException(websiteMediaValidationMessage(validation.reason));
  ...
  public async readMedia(shaOrId: string) {
    const row = (
      await sql<any>`select storage_key as "storageKey", media_type as "mediaType" from platform_website_media where deleted_at is null and (public_url=${`/api/v1/public/website/media/${shaOrId}`} or id::text=${shaOrId})`.execute(
        this.db,
      )
    ).rows[0];
