...
import {
  agentQuickActions,
  arabicAgentQuickActions,
  arabicGreeting,
  englishGreeting,
  greetingPeriod,
  tawseelhubAgentInstructions,
} from "./agent-instructions.js";

describe("Tawseelhub agent instructions", () => {
  it("keeps the public opening in Yousef's voice with all five quick actions", () => {
    expect(englishGreeting(new Date("2026-01-01T10:00:00.000Z"))).toBe(
      "Good afternoon, I’m Yousef, Tawseelhub AI Assistant. How can I help you today?",
    );
    expect(agentQuickActions).toEqual([
      "Send a Package",
      "Track Shipment",
      "Register as Trader",
    ...
    expect(arabicAgentQuickActions).toHaveLength(5);
  });
