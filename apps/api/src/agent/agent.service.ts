...
const reference = async (db: Kysely<DatabaseSchema>, sequence: string, prefix: string) => {
  const row = (
    await sql<{ n: string }>`select nextval(${sql.raw(`'${sequence}'`)})::text n`.execute(db)
  ).rows[0];
...
const businessInfoIntents = new Set<AgentIntent>([
  "general_question",
...
const workflowIntents = new Set<AgentIntent>([
  "customer_quote",
  "trader",
...
const trackingFollowUpPattern =
  /\b(status|delivered|update|when)\b|حال(?:ة|تها)|تسليم|توصيل|متى|تحديث/i;
// A universal "start over / show me the buttons again" escape hatch. Without
// this, typing "menu" while a question is pending (e.g. the contact-capture
// intro asking for a name) is silently swallowed as the literal answer to
// that question -- there was no way back to the quick actions once the
// conversation moved past its first message.
const menuRequestPattern =
  /^(?:menu|main menu|show menu|options|show options|القائمة|قائمة|الخيارات|الرئيسية)$/iu;
export function isMenuRequestText(text: string): boolean {
  return menuRequestPattern.test(text.trim());
}
const genericAnswerIntents = new Set<AgentIntent>(["general_question", "unknown"]);
const privateDirectoryOrCustomerInfo =
  /delivery companies|company directory|which traders|traders .*use|traders .*using|another customer|customer'?s information|customer'?s conversation|أسماء شركات التوصيل|شركات التوصيل المسجلة|أي تجار|معلومات عميل|محادثة عميل/i;
const quoteSlotOrder: Array<keyof AgentSlots> = [
  "requesterName",
  "requesterMobile",
  "pickupEmirate",
  "pickupArea",
...
const traderSlotOrder: Array<keyof AgentSlots> = [
  "storeName",
  "contactPerson",
  "mobileNumber",
  "email",
...
const demoSlotOrder: Array<keyof AgentSlots> = [
  "companyName",
  "contactPerson",
  "mobileNumber",
  "email",
...
const previousRequestPattern =
  /\b(last|previous|old)\b.*\b(request|shipment|quote|package|order)\b|what happened.*\b(request|shipment|quote|package|order)\b|شو صار.*(طلب|شحنة)|آخر طلب|اخر طلب/i;
const referencePattern = /\b(QTE-\d{6}|AGT-\d{6})\b/i;
const cancelHumanRequestPattern =
  /\b(never mind|cancel|continue with yousef|back to yousef|return to yousef|resume yousef|no human|no need|stop waiting)\b|خلاص|إلغاء|الغاء|ارجع|رجع يوسف|كمل مع يوسف|بدون موظف/i;
const normalizeInternationalMobile = (value: string | undefined | null) =>
  value?.replace(/[^\d]/g, "") || null;
const priceQuestionPattern =
  /\b(?:wha\s*tis|what\s+is|what'?s|what|how\s+much|cost|price|pricing|rate|charge|amount)\b.*\b(?:cost|price|pricing|rate|charge|amount)\b|\bhow\s+much\??$|\b(?:cost|price|pricing|rate|charge)\??$/i;
const arabicPriceQuestionPattern =
  /كم\s*(?:السعر|يكلف|التكلفة)|ما\s*(?:السعر|التكلفة)|السعر|تكلفة|بكم|كم\?/i;
const continueExistingPattern =
  /^(continue|continue previous|show my quote|show quote|old quote|previous quote|same quote|use existing|existing|تابع|كمل|اكمل|وريني السعر|اعرض السعر|الطلب السابق)$/i;
const newShipmentPattern =
  /^(new shipment|start new|start over|another package|new package|new quote|shipment new|شحنة جديدة|طلب جديد|ابدأ جديد|ابدا جديد|طرد جديد)$/i;
const skipPattern =
  /^(skip|no|not now|later|i don't have it|dont have it|لا|تخطي|بعدان|بعدين|ما عندي|ليس الآن)$/i;
export const arabicGeneralFallback =
  "Tawseelhub نظام تشغيل لشركات التوصيل في دولة الإمارات. يساعد في إدارة الطلبات والسائقين والتحصيل النقدي وتسويات التجار والتقارير وعلاقات التجار وطلبات الأسعار.";
const safeRulesClassifier = new RulesAgentModelProvider();
export const isCorruptedArabicText = (value: string) =>
  /\?{3,}/.test(value) && !/[\u0600-\u06ff]/.test(value);
export const isAgentPriceQuestionText = (value: string) =>
  priceQuestionPattern.test(value.trim()) || arabicPriceQuestionPattern.test(value.trim());
export const isAgentPlatformPricingQuestionText = (value: string) =>
  /system price|price .*system|system .*price|platform price|price .*platform|platform .*price|software price|price .*software|software .*price|product pricing|your prices?|your pricing|how (?:is|are).*prices?|subscription|monthly plan|pricing page|سعر النظام|سعر استخدام النظام|سعر.*النظام|النظام.*سعر|اشتراك|باقة|باقات|رسوم النظام|تكلفة النظام/i.test(
    value.trim(),
  );
export const isAgentAnyPricingTopicText = (value: string) => {
  if (isAgentPlatformPricingQuestionText(value)) return true;
  const text = value.trim();
  const asksDeliveryQuote =
    /shipment|package|parcel|delivery price|delivery quote|send|pickup|drop(?:off)?|شحنة|طرد|سعر توصيل|عرض سعر توصيل|استلام|توصيل/i.test(
      text,
    );
  return isAgentPriceQuestionText(text) && !asksDeliveryQuote;
};
export const isAgentAffirmativeText = (value: string) =>
  /^(yes|y|ok|okay|sure|please|confirm|نعم|اي|إي|ايوه|أيوه|تمام|اوكي|أوكي|أكيد|اكيد)$/i.test(
    value.trim(),
  );
export const isAgentDeductionQuestionText = (value: string) => {
  ...
  const feeWords =
    /deduct|deduction|charge|fee|fees|subscription|monthly|cost|خصم|تخصم|تخصمون|رسوم|اشتراك|تكلفة|تحاسب|تدفع/.test(
      lower,
    );
  return asksWhy && feeWords;
};
export const isAgentConfusionText = (value: string) =>
  /not clear|unclear|confusing|confused|i do not understand|i don't understand|what do you mean|الكلام غير مفهوم|غير مفهوم|مش فاهم|ما فهمت|شو تقصد/i.test(
    value.trim(),
  );
export const isAgentTraderUsageQuestionText = (value: string) => {
  ...
  const usageWords =
    /use|work|benefit|system|platform|manage|يستخدم|يستفيد|يشتغل|النظام|المنصة|يدير/.test(lower);
  return mentionsTrader && asksHow && usageWords;
};
export const isAgentFeatureExplanationText = (value: string) =>
  /driver management|manage drivers|drivers|cod|cash on delivery|collections|settlements|reports|payroll|إدارة السائقين|ادارة السائقين|السائقين|المندوبين|تحصيل|COD|التسويات|تسويات|التقارير|تقارير|الرواتب|رواتب/i.test(
    value.trim(),
  );
export const isAgentTraderExplanationChoiceText = (value: string) =>
  /^(trader|traders|seller|sellers|merchant|merchants|store|stores|the trader|for traders|التاجر|التجار|للتاجر|للتجار|تاجر|تجار|المتجر|المتاجر)$/i.test(
    value.trim(),
  );
export const isAgentExplainOnlyText = (value: string) =>
  /explain only|just explain|no request|no order|what request|اشرح فقط|فقط اشرح|اشرح النظام|انت فقط اشرح|أنت فقط اشرح|لا يوجد طلب|لايوجد طلب|ما في طلب|اي طلب|أي طلب|وانت مالك|وأنت مالك/i.test(
    value.trim(),
  );
const traderRegistrationChoicePattern =
  /trader registration|register as trader|تسجيل.*تاجر|التسجيل كتاجر|كتاجر|تاجر/i;
const emailAddressPattern = /[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/i;
export const publicAgentLabel = (value: unknown): string =>
  String(value ?? "")
    .replace(/_/g, " ")
    .replace(/\s+/g, " ")
    .trim()
...
export function generalKnowledgeContent(
  language: AgentLanguage,
  storedContent: string | undefined,
): string {
  if (language === "ar" && (storedContent === undefined || isCorruptedArabicText(storedContent)))
    return arabicGeneralFallback;
  return storedContent ?? "I do not have confirmed information for that yet.";
}
export function contextualGeneralFollowUpResponse(
  text: string,
  language: AgentLanguage,
  ...
  | {
      ...
      intent: AgentIntent;
      status: string;
      structured: { suppressReturningAcknowledgement: true; state: AgentState };
    }
  | undefined {
  if (
    isAgentFeatureExplanationText(text) &&
    !/(demo|عرض تجريبي|احجز|book|submit|send request|أرسل طلب|ارسل طلب)/i.test(text)
  ) {
    const {
      pendingAction: _pendingAction,
      lastAskedSlot: _lastAskedSlot,
      pendingGeneralFollowUp: _pendingGeneralFollowUp,
      ...baseState
    } = currentState;
    ...
    return {
      ...
      structured: {
        suppressReturningAcknowledgement: true,
        state: {
          ...baseState,
          lastBusinessIntent: "product_feature_question",
  ...
  if (
    previousState.lastAskedSlot &&
    previousState.lastBusinessIntent &&
    workflowIntents.has(previousState.lastBusinessIntent) &&
    isAgentExplainOnlyText(text)
  ) {
    const {
      pendingAction: _pendingAction,
      lastAskedSlot: _lastAskedSlot,
  ...
  if (
    (previousState.pendingGeneralFollowUp === "public_explanation" ||
      previousState.pendingGeneralFollowUp === "feature_choice") &&
    isAgentTraderExplanationChoiceText(text)
  ) {
    const {
      pendingAction: _pendingAction,
      lastAskedSlot: _lastAskedSlot,
  ...
  if (previousState.pendingAction && isAgentConfusionText(text)) {
    ...
    return {
      ...
      intent: "general_question",
      status: "waiting_for_user",
      structured: { suppressReturningAcknowledgement: true, state: baseState },
    };
  }
  if (isAgentDeductionQuestionText(text)) {
    ...
    return {
      ...
      intent: "general_question",
      status: "waiting_for_user",
      structured: { suppressReturningAcknowledgement: true, state: clearedState },
    };
  }
  if (isAgentAnyPricingTopicText(text)) {
    ...
    return {
      ...
      structured: {
        suppressReturningAcknowledgement: true,
        state: { ...baseState, lastBusinessIntent: "general_question" },
      },
    };
  }
  if (
    previousState.pendingGeneralFollowUp === "feature_choice" &&
    traderRegistrationChoicePattern.test(text) &&
    !isAgentTraderExplanationChoiceText(text)
  ) {
    const {
      pendingAction: _pendingAction,
    ...
    return {
      ...
      structured: {
        suppressReturningAcknowledgement: true,
        state: { ...baseState, pendingGeneralFollowUp: "trader_registration_explained" },
      },
    };
  }
  if (
    previousState.pendingGeneralFollowUp === "trader_registration_explained" &&
    isAgentAffirmativeText(text)
  ) {
    const {
      pendingAction: _pendingAction,
      lastAskedSlot: _lastAskedSlot,
  ...
  if (previousState.pendingGeneralFollowUp && isAgentAffirmativeText(text)) {
    ...
    return {
      ...
      structured: {
        suppressReturningAcknowledgement: true,
        state: { ...currentState, pendingGeneralFollowUp: "feature_choice" },
      },
    };
  }
  return undefined;
...
function featureExplanationText(text: string, language: AgentLanguage): string {
  ...
  if (
    /driver management|manage drivers|drivers|إدارة السائقين|ادارة السائقين|السائقين|المندوبين/.test(
      lower,
    )
  ) {
    return language === "ar"
 ...
 * tracking. Company/store name and email used to be asked unconditionally
 * before ANY workflow began, but no current workflow actually needs that
 * from this generic gate: `customer_quote` never asks for a company at all,
 * and `trader`/`delivery_company_demo` already ask for storeName/companyName
 * (and email) themselves, via their own required-slot list, the moment they
 * actually need it -- asking here first was pure redundant friction, and for
 * an anonymous shipment-tracking customer it made no sense at all.
 *
 * Once name + mobile are collected, if a real workflow was already pending
 * (`pendingWorkflowIntent`), the caller resumes that workflow's own next
 * question directly -- resumeIntent signals that. Only a bare, no-workflow
 * conversation gets the generic "how can I help" acknowledgement.
 */
export function publicConversationIntroStep(
  text: string,
  language: AgentLanguage,
  state: AgentState,
  pendingWorkflowIntent?: AgentIntent,
):
  | {
      content: string;
      intent: AgentIntent;
      status: string;
      structured: { state: AgentState };
  ...
  const isNonAnswer =
    !answer || isQuestion || /^(hi|hello|hey|مرحبا|هلا|السلام عليكم|كيفك|كيف الحال)$/i.test(answer);
  const normalizedMobileAnswer = answer.replace(/\D/g, "");
  const isUaeMobileAnswer =
    /^05\d{8}$/.test(normalizedMobileAnswer) ||
    /^5\d{8}$/.test(normalizedMobileAnswer) ||
    /^9715\d{8}$/.test(normalizedMobileAnswer);
  ...
  if (previousMissing === "requesterMobile") {
    if (isUaeMobileAnswer) {
      ...
      return {
        content:
          language === "ar"
            ? "رقم الهاتف غير واضح. اكتب رقم موبايل إماراتي مثل 0501234567."
            : "The mobile number is not clear. Please enter a UAE mobile like 0501234567.",
        intent: "general_question",
        status: "waiting_for_user",
        structured: {
          state: {
  ...
  if (!hasMobile) {
    return {
      content:
        language === "ar"
          ? "ما رقم الهاتف المتحرك في الإمارات؟"
          : "What UAE mobile number should we use?",
      intent: "general_question",
      status: "waiting_for_user",
      structured: {
        state: {
  ...
  if (previousMissing === "contactName" || previousMissing === "requesterMobile") {
    if (pendingWorkflowIntent && pendingWorkflowIntent !== "general_question") {
      const { lastAskedSlot: _lastAskedSlot, ...resumedState } = {
        ...state,
        slots,
        lastBusinessIntent: pendingWorkflowIntent,
      };
      return {
        content: "",
        intent: pendingWorkflowIntent,
        status: "waiting_for_user",
        structured: { state: resumedState },
        resumeIntent: pendingWorkflowIntent,
      };
    }
    const { lastAskedSlot: _lastAskedSlot, ...nextState } = {
      ...state,
      slots,
    ...
    return {
      content:
        language === "ar"
          ? "شكراً، تم حفظ بيانات التواصل. كيف يمكنني مساعدتك الآن؟"
          : "Thanks, I saved the contact details. How can I help you now?",
      intent: "general_question",
      status: "waiting_for_user",
...
export class AgentService {
  ...
  public async createWebsiteConversation(
    language?: AgentLanguage,
    visitorId?: string,
    visitorIp?: string,
  ) {
    ...
    if (!settings.agentEnabled || !settings.websiteChatEnabled)
      throw new BadRequestException("agent_disabled");
  ...
  public async simulateWhatsApp(input: {
    sender: string;
    message: string;
  ...
  public async receiveWhatsAppWebhook(
    body: unknown,
    rawBody: Buffer | undefined,
    signature: string | undefined,
  ) {
    const settings = await this.settings();
    if (!settings.whatsappAgentEnabled || settings.whatsappProvider === "disabled")
      throw new BadRequestException("whatsapp_disabled");
    ...
    return this.receiveWhatsAppMessages(
      ...
      provider.normalizeWebhook(body),
    );
  }

  public async receiveWhatsAppMessages(
    providerName: string,
    messages: Array<{
      eventId: string;
      messageId: string;
    ...
    for (const message of messages) {
      const existing = (
        await sql<{
          id: string;
        }>`select id from platform_agent_messages where provider=${providerName} and provider_message_id=${message.messageId} limit 1`.execute(
          this.db,
        )
      ).rows[0];
      if (existing) {
      ...
      const result = await this.handleInbound(conversation, {
        channel: "whatsapp",
    ...
    await sql`update platform_agent_settings set whatsapp_last_webhook_at=now(), whatsapp_last_error_code=null where id=true`.execute(
      this.db,
    );
  ...
  private async findOrCreateWhatsAppConversation(
    ...
    return (
      ...
      >`select * from platform_agent_conversations where reference_number=${ref}`.execute(this.db)
    ).rows[0]!;
  }
  ...
  private async handleInbound(
    ...
    if (input.inboundMessageId && state.seenInboundMessageIds?.includes(input.inboundMessageId))
      return this.publicConversation(conversation);
    state.seenInboundMessageIds = [
      ...(state.seenInboundMessageIds ?? []),
      ...(input.inboundMessageId ? [input.inboundMessageId] : []),
    ].slice(-50);
    await this.appendMessage(String(conversation.id), "user", input.text, undefined, {
    ...
    if (
      ...
      const content =
        language === "ar"
          ? "تم، يوسف متاح مرة أخرى. كيف يمكنني مساعدتك الآن؟"
          : "Done — Yousef is available again. How can I help you now?";
      await sql`
        update platform_agent_conversations
      ...
      const updated = (
        await sql<
          Record<string, unknown>
        >`select * from platform_agent_conversations where id=${conversation.id}::uuid`.execute(
          this.db,
        )
      ).rows[0]!;
      return {
        ...(await this.publicConversation(updated)),
        reply: content,
        intent: "general_question",
        language,
      };
    ...
    if (frame.decision === "privacy_blocked") {
      ...
      await this.appendMessage(
        ...
        "assistant",
        content,
        { conversationFrame: frame, state: framedState },
        {
          channel: input.channel,
          direction: "outbound",
          ...(input.provider === undefined ? {} : { provider: input.provider }),
      ...
      await sql`update platform_agent_conversations set language=${language},current_intent='general_question',status='waiting_for_user',audience=${framedState.audience ?? "unknown"},state=${JSON.stringify(framedState)}::jsonb,updated_at=now(),last_message_at=now() where id=${conversation.id}::uuid`.execute(
        this.db,
      );
      const updated = (
        await sql<
    ...
    if (
      ...
      (!settings.whatsappAgentEnabled || settings.whatsappProvider === "disabled")
    ) {
      await sql`update platform_agent_conversations set state=${JSON.stringify(state)}::jsonb, review_status='open', last_customer_message_at=now(), last_channel='whatsapp', updated_at=now(), last_message_at=now() where id=${conversation.id}::uuid`.execute(
        this.db,
      );
      const updated = (
        await sql<
    ...
    if (isMenuRequestText(input.text)) {
      const language = input.language ?? (conversation.language as AgentLanguage);
      const welcome = this.welcome(language);
      const quickActions = language === "ar" ? agentQuickActionsArabic : agentQuickActions;
      const resetState: AgentState = { slots: {}, audience: "unknown", discussedTopics: [] };
      await this.appendMessage(
        String(conversation.id),
        "assistant",
        welcome,
        { quickActions, state: resetState },
        {
          channel: input.channel,
          direction: "outbound",
          ...(input.provider === undefined ? {} : { provider: input.provider }),
      ...
      await sql`update platform_agent_conversations set language=${language},current_intent='general_question',status='waiting_for_user',audience='unknown',state=${JSON.stringify(resetState)}::jsonb,updated_at=now(),last_message_at=now() where id=${conversation.id}::uuid`.execute(
        this.db,
      );
      const updated = (
        await sql<
    ...
    // Preserve the workflow this turn's frame decision recognized (e.g. an
    // explicit "Track Shipment" start) across the contact-capture detour --
    // otherwise `conversationFrame` never gets applied while intro is
    // short-circuiting the turn, `decideNextFrame` sees no active workflow
    // on the next message, and the original intent is silently lost.
    const introState = stateWithFrame(state, frame);
    const intro = publicConversationIntroStep(
      input.text,
      input.language ?? (conversation.language as AgentLanguage),
      introState,
      agentIntentFromWorkflow(frame.workflow),
    );
    if (intro) {
      const language = input.language ?? (conversation.language as AgentLanguage);
      if (intro.resumeIntent) {
        // Name + mobile are collected -- resume the workflow that was
        // already pending (tracking/trader/demo/quote/handoff) by calling
        // its OWN next-question logic directly, rather than hardcoding a
        // duplicate of each workflow's first question here (which would
        // drift out of sync with quoteStep/traderStep/demoStep/trackingStep
        // the moment any of their required-slot lists changed).
        const resumed = (await this.nextResponse(
          String(conversation.id),
          intro.resumeIntent,
          language,
          intro.structured.state,
          "",
          input.channel,
          settings,
        )) as {
          ...
          intent?: AgentIntent;
          status?: string;
          structured?: Record<string, unknown> & { state?: AgentState };
        };
        const mergedState = resumed.structured?.state ?? intro.structured.state;
        const identity = this.identityFromState(mergedState);
        const persistedIntent = persistableAgentConversationIntent(
          resumed.intent ?? intro.resumeIntent,
        );
        await this.appendMessage(
          String(conversation.id),
          "assistant",
        ...
        return {
          ...
          intent: resumed.intent ?? intro.resumeIntent,
          language,
        };
      }
    ...
    if (state.lastAskedSlot && this.isClarification(input.text)) {
      const contextualClarification = contextualGeneralFollowUpResponse(
        input.text,
        input.language ?? (conversation.language as AgentLanguage),
        state,
        state,
      ...
      if (contextualClarification) {
        await this.appendMessage(
          String(conversation.id),
          "assistant",
          contextualClarification.content,
          contextualClarification.structured,
          {
            channel: input.channel,
            direction: "outbound",
            ...(input.provider === undefined ? {} : { provider: input.provider }),
        ...
        return {
          ...(await this.publicConversation(updated)),
          reply: contextualClarification.content,
          intent: "general_question",
          language: input.language ?? (conversation.language as AgentLanguage),
        };
      }
      const language = input.language ?? (conversation.language as AgentLanguage);
      ...
      await this.appendMessage(
        ...
        "assistant",
        response.content,
        { clarificationFor: state.lastAskedSlot },
        {
          channel: input.channel,
          direction: "outbound",
          ...(input.provider === undefined ? {} : { provider: input.provider }),
      ...
      await sql`update platform_agent_conversations set language=${language},current_intent='clarification',status='waiting_for_user',state=${JSON.stringify(updatedState)}::jsonb,updated_at=now(),last_message_at=now() where id=${conversation.id}::uuid`.execute(
        this.db,
      );
      const updated = (
        await sql<
    ...
    try {
      model = await this.model.classifyAndExtract({
        text: input.text,
        language: input.language ?? (conversation.language as AgentLanguage),
        previousIntent: this.previousIntentForClassification(
          conversation.current_intent as AgentIntent,
          state,
        ),
        state,
      });
      ...
      try {
        model = await safeRulesClassifier.classifyAndExtract({
          text: input.text,
          language: input.language ?? (conversation.language as AgentLanguage),
          previousIntent: this.previousIntentForClassification(
            conversation.current_intent as AgentIntent,
        ...
        this.logger.error(
          {
            code:
              (rulesError as { code?: string; message?: string }).code ??
              (rulesError as { message?: string }).message?.slice(0, 80) ??
              "rules_failed",
          },
        ...
        const safe =
          input.channel === "website"
            ? offline
              ? this.humanUnavailableAskForContactMessage(language, state)
              : this.humanWaitingMessage(language)
            : "I'm unable to complete that request right now. You can try again, or I can pass your request to the Tawseelhub team.";
        const fallbackState = offline ? this.humanUnavailableContactState(state) : state;
        await this.appendMessage(
          String(conversation.id),
          "assistant",
          safe,
          { providerError: true, rulesError: true, state: fallbackState },
          {
            channel: input.channel,
            direction: "outbound",
            ...(input.provider === undefined ? {} : { provider: input.provider }),
        ...
        if (input.channel === "website" && !offline)
          await this.markWebsiteHumanRequested(
            String(conversation.id),
            String(conversation.conversation_mode ?? "ai_active"),
            "Model provider and rules fallback failed",
          );
        if (offline)
          await sql`update platform_agent_conversations set current_intent='handoff',status='waiting_for_user',requester_type='unknown',audience=${fallbackState.audience ?? "unknown"},state=${JSON.stringify(fallbackState)}::jsonb,review_status=case when review_status='new' then 'follow_up' else review_status end,last_customer_message_at=now(),last_channel=${input.channel},updated_at=now(),last_message_at=now() where id=${conversation.id}::uuid`.execute(
            this.db,
          );
        const updated = (
          await sql<
    ...
    const turnIntent =
      isAgentTraderUsageQuestionText(input.text) || isAgentFeatureExplanationText(input.text)
        ? "product_feature_question"
        : this.resolveTurnIntent(input.text, frameIntent, state);
    ...
    let nextState = this.enrichState(input.text, turnIntent, {
      ...frameScopedState,
      slots: this.applySequentialWorkflowAnswer(
        input.text,
        turnIntent,
        frameScopedState,
        mergedSlots,
    ...
    const trackingFollowUp = this.trackingFollowUpResponse(
      input.text,
      model.language,
      state,
      nextState,
    );
    const contextualGeneralFollowUp = trackingFollowUp
      ? undefined
      : contextualGeneralFollowUpResponse(input.text, model.language, state, nextState);
    const interruption =
      trackingFollowUp || contextualGeneralFollowUp
        ? undefined
        : await this.workflowInterruptionResponse(
            String(conversation.id),
            input.text,
            model.language,
            state,
    ...
    if (trackingFollowUp) response = trackingFollowUp;
    ...
    else if (state.pendingAction && model.wantsConfirmation)
      response = await this.executePending(
        String(conversation.id),
      ...
      response =
        (await this.previousRequestQuestionResponse(
          String(conversation.id),
          input.text,
          model.language,
          nextState,
        )) ??
        (await this.nextResponse(
          String(conversation.id),
          turnIntent,
          model.language,
          nextState,
    ...
    const acknowledged = response.structured?.suppressReturningAcknowledgement
      ? response.content
      : await this.returningCustomerAcknowledgement(
          String(conversation.id),
          model.language,
          state,
          mergedState,
    ...
    const isWebsiteHandoff =
      input.channel === "website" && response.structured?.websiteHumanRequested === true;
    const reviewStatusOverride =
      typeof response.structured?.reviewStatus === "string"
        ? response.structured.reviewStatus
        : null;
    await sql`update platform_agent_conversations set language=${model.language},current_intent=${persistedIntent},status=${response.status ?? "waiting_for_user"},requester_type=${this.requesterType(responseIntent)},audience=${mergedState.audience ?? "unknown"},customer_name=${identity.name},mobile_number=${identity.mobileOriginal},mobile_number_normalized=${identity.mobileNormalized},email=${identity.email},operational_classification=${classification},state=${JSON.stringify(mergedState)}::jsonb,conversation_mode=case when ${isWebsiteHandoff}::boolean then 'paused' else conversation_mode end,review_status=case when ${reviewStatusOverride}::text is not null then ${reviewStatusOverride}::text when ${isWebsiteHandoff}::boolean and review_status='new' then 'open' else review_status end,mode_changed_at=case when ${isWebsiteHandoff}::boolean then now() else mode_changed_at end,updated_at=now(),last_message_at=now() where id=${conversation.id}::uuid`.execute(
      this.db,
    );
    if (isWebsiteHandoff)
      await this.recordModeHistory(
        String(conversation.id),
        String(conversation.conversation_mode ?? "ai_active"),
        "paused",
        null,
        "Human requested from website chat",
      );
    const assistantMessageId = await this.appendMessage(
      String(conversation.id),
      "assistant",
      acknowledged,
      response.structured,
    ...
    if (input.channel === "whatsapp" && input.provider) {
      ...
      const send = await provider.sendText(
        String(conversation.mobile_number_normalized ?? ""),
        acknowledged,
      );
      await sql`
      ...
      `.execute(this.db);
      await sql`update platform_agent_settings set whatsapp_last_outbound_at=case when ${send.status}<>'failed' then now() else whatsapp_last_outbound_at end, whatsapp_last_error_code=${send.failureCode ?? null} where id=true`.execute(
        this.db,
      );
    }
    const updated = (
    ...
    return {
      ...(await this.publicConversation(updated)),
      reply: acknowledged,
      intent: responseIntent ?? turnIntent,
  ...
  private async nextResponse(
    ...
    currentStatus = "",
    channel?: AgentChannel,
    settings?: { humanHandoffEnabled?: boolean },
  ) {
    if (socialIntents.has(intent)) return this.socialResponse(intent, language);
    if (
      currentStatus === "completed" &&
      state.lastBusinessIntent === "customer_quote" &&
      !state.pendingAction
    )
      return this.completedQuoteFollowUp(language);
    if (intent === "handoff")
      return this.prepareHandoff(
        conversationId,
        state,
        language,
        "requested_by_user",
  ...
  private quoteStep(state: AgentState, language: AgentLanguage) {
    ...
    const required: Array<[keyof AgentSlots, string, string]> = [
      [
        ...
        "ما الاسم المطلوب لطلب السعر؟",
      ],
      [
        "requesterMobile",
        "What UAE mobile number should we use for the quote?",
        "ما رقم الهاتف المتحرك في الإمارات؟",
      ],
    ...
    if (missing)
      return {
        content: language === "ar" ? missing[2] : missing[1],
        intent: "customer_quote" as const,
        status: "waiting_for_user",
        structured: { state: { ...state, slots, lastAskedSlot: missing[0] } },
      };
    if (!slots.deliveryAddress && !state.deliveryAddressSkipped) {
      ...
      return {
        ...
        intent: "customer_quote" as const,
        status: "waiting_for_user",
        structured: { state: { ...state, slots, lastAskedSlot: "deliveryAddress" as const } },
      };
    }
    const summary = this.quoteSummary(slots);
    const content =
      language === "ar"
        ? `لدي التفاصيل الأساسية. هل ترغب أن أطلب السعر الآن؟\n${this.lines(summary)}`
        : `I have the core details. Would you like me to request the quote now?\n${this.lines(summary)}`;
    return {
      content,
      intent: "customer_quote" as const,
      status: "action_pending",
      structured: {
        state: { ...state, slots, pendingAction: { type: "calculate_customer_quote", summary } },
      },
    };
  }
  ...
  private traderStep(state: AgentState, language: AgentLanguage) {
    const slots = {
      primaryCategory: "general_trading",
      monthlyOrderRange: "under_100",
    ...
    const required: Array<[keyof AgentSlots, string, string]> = [
      ...
      [
        "pickupEmirate",
        "Which emirate is the main pickup location in?",
        "في أي إمارة يكون موقع الاستلام الرئيسي؟",
      ],
      [
        "pickupBusinessArea",
        "What pickup area should we list for the Trader application?",
        "ما منطقة الاستلام للتاجر؟",
      ],
    ];
    const missing = required.find(([key]) => slots[key] === undefined || slots[key] === "");
    if (missing)
      return {
        content: language === "ar" ? missing[2] : missing[1],
        intent: "trader" as const,
        status: "waiting_for_user",
        structured: { state: { ...state, slots, lastAskedSlot: missing[0] } },
      };
    const summary = compact({
      store: slots.storeName,
      contact: slots.contactPerson,
      mobile: slots.mobileNumber,
      email: slots.email,
      pickup: `${slots.pickupBusinessArea}, ${slots.pickupEmirate}`,
      existingDeliveryCompany: slots.hasExistingDeliveryCompany
        ? (slots.existingDeliveryCompanyName ?? "Provided by Trader")
        : "Needs Delivery Company",
      channels: slots.channels?.map((c) => c.type).join(", "),
    });
    const content =
      language === "ar"
        ? `هل أرسل طلب تسجيل التاجر بهذه التفاصيل؟\n${this.lines(summary)}`
        : `Should I submit the Trader application with these details?\n${this.lines(summary)}`;
    return {
      content,
      intent: "trader" as const,
      status: "action_pending",
      structured: {
        state: { ...state, slots, pendingAction: { type: "submit_trader_application", summary } },
      },
    };
  }
  ...
  private demoStep(state: AgentState, language: AgentLanguage) {
    ...
    if (missing)
      return {
        content: language === "ar" ? missing[2] : missing[1],
        intent: "delivery_company_demo" as const,
        status: "waiting_for_user",
        structured: { state: { ...state, slots, lastAskedSlot: missing[0] } },
      };
    const summary = compact({
      ...
      mobile: slots.mobileNumber,
      email: slots.email,
      emirate: slots.emirate,
      drivers: slots.approximateDriverCount,
    });
    const content =
      language === "ar"
        ? `هل أرسل طلب العرض التجريبي بهذه التفاصيل؟\n${this.lines(summary)}`
        : `Should I submit the demo request with these details?\n${this.lines(summary)}`;
    return {
      content,
      intent: "delivery_company_demo" as const,
      status: "action_pending",
      structured: {
        state: { ...state, slots, pendingAction: { type: "submit_demo_request", summary } },
      },
    };
  }
  ...
  private async trackingStep(state: AgentState, language: AgentLanguage) {
    ...
    if (tracking?.verificationToken) {
      // Never ask the customer to repeat a mobile number they already gave
      // us earlier in this same conversation (as their own contact mobile).
      // Try it once, silently -- only fall back to asking if that attempt
      // does not verify.
      const alreadyKnownMobile = !tracking.autoMobileAttempted
        ? (slots.requesterMobile ?? slots.mobileNumber ?? slots.mobile)
        : undefined;
      const mobile = slots.trackingMobileNumber ?? alreadyKnownMobile;
      if (!mobile) {
        return {
          content:
            language === "ar"
          ...
          status: "waiting_for_user",
          structured: {
            state: { ...state, tracking, lastAskedSlot: "trackingMobileNumber" as const },
          },
        };
      }
      const trackingBeforeAttempt = alreadyKnownMobile
        ? { ...tracking, autoMobileAttempted: true }
        : tracking;
      const outcome = await this.tracking.verifyAmbiguousShipment(
        tracking.verificationToken,
        mobile,
        language,
      );
      if (outcome.result === "verified")
        return this.trackingResultResponse(state, language, outcome.tracking);
      ...
      const failedAttempts = alreadyKnownMobile
        ? (tracking.failedMobileAttempts ?? 0)
        : (tracking.failedMobileAttempts ?? 0) + 1;
      if (failedAttempts >= AgentService.TRACKING_MAX_FAILED_MOBILE_ATTEMPTS)
        return this.trackingSupportResponse(state, language);
      ...
      return {
        ...
        structured: {
          state: {
            ...
            tracking: { ...trackingBeforeAttempt, failedMobileAttempts: failedAttempts },
    ...
    if (!airwayBill) {
      return {
        content:
          language === "ar"
            ? "الرجاء إدخال رقم بوليصة الشحن أو رقم طلب Tawseelhub، مثل X123456 أو ORD-000116."
            : "Please enter your Airway Bill or Tawseelhub Order Number, for example X123456 or ORD-000116.",
        intent: "shipment_tracking" as const,
        status: "waiting_for_user",
        structured: { state: { ...state, lastAskedSlot: "trackingAirwayBill" as const } },
      };
    }
    const outcome = await this.tracking.lookupByAirwayBill(airwayBill, language);
    if (outcome.result === "verified")
      return this.trackingResultResponse(state, language, outcome.tracking);
    ...
    return {
      ...
      status: "waiting_for_user",
      structured: {
        state: { ...state, slots: remainingSlots, lastAskedSlot: "trackingAirwayBill" as const },
      },
    };
  }
  ...
  private trackingResultResponse(
    ...
    const {
      trackingAirwayBill: _trackingAirwayBill,
      trackingMobileNumber: _trackingMobileNumber,
      ...remainingSlots
    } = state.slots;
    ...
    return {
      ...
      structured: {
        state: {
          ...baseState,
          slots: remainingSlots,
          tracking: { result: tracking, startedAt: new Date().toISOString() },
        },
      },
    };
  }
   ...
   */
  private trackingFollowUpResponse(
    text: string,
    language: AgentLanguage,
    previousState: AgentState,
    currentState: AgentState,
    ...
    return {
      ...
      structured: {
        state: {
          ...currentState,
          tracking: { result, startedAt: previousState.tracking?.startedAt },
        },
      },
    };
  }
  ...
  private async executePending(
    conversationId: string,
    type: NonNullable<AgentState["pendingAction"]>["type"],
    state: AgentState,
  ) {
    try {
      if (type === "calculate_customer_quote") return await this.createQuote(conversationId, state);
      if (type === "submit_trader_application")
        return await this.createTraderApplication(conversationId, state);
      if (type === "submit_demo_request")
        return await this.createDemoRequest(conversationId, state);
      return await this.createHandoff(conversationId, state, "confirmed_by_user");
    } catch (error) {
      await sql`insert into platform_agent_actions(conversation_id,action_type,status,request_snapshot,safe_error_code) values(${conversationId}::uuid,${type},'failed',${JSON.stringify(state.pendingAction?.summary ?? {})}::jsonb,${(error as { message?: string }).message?.slice(0, 80) ?? "action_failed"})`.execute(
        this.db,
      );
      return {
        content:
          "I could not complete that request right now. You can try again or I can pass it to the Tawseelhub team.",
  ...
  private async createQuote(conversationId: string, state: AgentState) {
    const slots = state.slots;
    const existing = (
      await sql<{ reference_number: string; status: string; offer_count: string }>`
    ...
    if (existing) {
      const offerText =
        Number(existing.offer_count) > 0
          ? "The available quote options are already saved with that request."
          : "This shipment requires a custom quotation.";
      return {
        content: `Your quote request has already been received. Reference: ${existing.reference_number}\n${offerText}\nDo you have another question, or how can I help you now?`,
        intent: "customer_quote" as const,
        status: "completed",
        structured: { state: this.completedWorkflowState(state) },
      };
    ...
    const recipientMobile = this.formatPublicUaeMobile(
      String(slots.recipientMobile ?? slots.requesterMobile),
    );
    const payload = {
      ...
      deliveryAddress: String(
        slots.deliveryAddress ?? `${slots.deliveryArea}, ${slots.deliveryEmirate}`,
      ),
    ...
    await sql`insert into platform_agent_actions(conversation_id,action_type,status,request_snapshot,response_snapshot) values(${conversationId}::uuid,'calculate_customer_quote','completed',${JSON.stringify(this.redactSlots(slots))}::jsonb,${JSON.stringify({ quoteReference: result.quoteReference, quoteType: result.quoteType, status: result.status, offerCount: result.offers.length })}::jsonb)`.execute(
      this.db,
    );
    const quote = (
      await sql<{
        id: string;
      }>`select id from platform_customer_quote_requests where reference_number=${result.quoteReference}`.execute(
        this.db,
      )
    ).rows[0];
    if (quote)
  ...
  private async createTraderApplication(conversationId: string, state: AgentState) {
    ...
    const traderPayload = {
      ...
      existingDeliveryCompanyName: s.hasExistingDeliveryCompany
        ? (s.existingDeliveryCompanyName ?? "Provided by Trader")
        : undefined,
      consent: true,
      landingPage: "/agent",
    } as any;
    ...
    return {
      content: `Your Trader application has been received. Reference: ${result.referenceNumber}. Status: Pending Verification.\nDo you have another question, or how can I help you now?`,
      intent: "trader" as const,
      status: "completed",
      structured: { state: this.completedWorkflowState(state) },
    };
  ...
  private async createDemoRequest(conversationId: string, state: AgentState) {
    ...
    const demoPayload = {
      ...
      ...(s.approximateDriverCount === undefined
        ? {}
        : { approximateDriverCount: s.approximateDriverCount }),
      ...(s.approximateMonthlyOrders === undefined
        ? {}
        : { approximateMonthlyOrders: s.approximateMonthlyOrders }),
      ...(s.approximateTraderCount === undefined
        ? {}
        : { approximateTraderCount: s.approximateTraderCount }),
      ...
      featuresOfInterest: s.featuresOfInterest?.length
        ? (s.featuresOfInterest as any)
        : ["order_management", "driver_management", "cod_collections"],
      consent: true,
      landingPage: "/agent",
    } as any;
    ...
    return {
      content: `Your demo request has been received. Reference: ${result.referenceNumber}.\nDo you have another question, or how can I help you now?`,
      intent: "delivery_company_demo" as const,
      status: "completed",
      structured: { state: this.completedWorkflowState(state) },
    };
  ...
  private async prepareHandoff(
    ...
    if (channel === "website") {
      if (!humanHandoffEnabled)
        return this.offlineHumanFollowUpStep(conversationId, state, language, reason);
      return {
        content: this.humanWaitingMessage(language),
        intent: "handoff" as const,
        status: "waiting_for_user",
        structured: { state: this.completedWorkflowState(state), websiteHumanRequested: true },
      };
    }
    const summary = compact({
      reason,
      name: state.slots.contactName ?? state.slots.requesterName ?? state.slots.contactPerson,
      mobile: state.slots.mobile ?? state.slots.mobileNumber ?? state.slots.requesterMobile,
      email: state.slots.email ?? state.slots.requesterEmail,
    });
    ...
    return {
      ...
      intent: "handoff" as const,
      status: "action_pending",
      structured: { state: { ...state, pendingAction: { type: "create_handoff", summary } } },
    };
  }
  ...
  private async offlineHumanFollowUpStep(
    conversationId: string,
    state: AgentState,
    language: AgentLanguage,
    reason: string,
    ...
    if (!name)
      return {
        ...
        structured: {
          state: {
            ...state,
            lastBusinessIntent: "handoff" as const,
            lastAskedSlot: "contactName" as const,
          },
        },
      };
    if (!mobile)
      return {
        content:
          language === "ar"
            ? "لا يوجد موظف متاح الآن. ما رقم الهاتف المتحرك الذي يمكن لفريق العمليات التواصل معك عليه؟"
            : "No human agent is available right now. What mobile number should our operations team use to contact you?",
        intent: "handoff" as const,
        status: "waiting_for_user",
        structured: {
          state: {
            ...state,
            slots: { ...slots, contactName: name, requesterName: slots.requesterName ?? name },
            lastBusinessIntent: "handoff" as const,
            lastAskedSlot: "mobile" as const,
          },
        },
      };
    return this.createHandoff(
      conversationId,
      {
        ...
        slots: {
          ...
          contactName: name,
          mobile,
          mobileNumber: slots.mobileNumber ?? mobile,
          requesterMobile: slots.requesterMobile ?? mobile,
        },
      },
      reason,
      {
  ...
  private async createHandoff(
    ...
    state: AgentState,
    reason: string,
    options?: { content?: string; reviewStatus?: "open" | "follow_up" },
  ) {
    const existing = (
      await sql<{ reference_number: string }>`
    ...
    if (existing) {
      return {
        content:
          options?.content ??
          `I already passed this to the Tawseelhub team. Reference: ${existing.reference_number}.\nDo you have another question, or how can I help you now?`,
        intent: "handoff" as const,
        status: "handed_off",
        structured: {
          state: this.completedWorkflowState(state),
    ...
    const inserted = await sql<{
      id: string;
    }>`insert into platform_agent_handoffs(reference_number,conversation_id,reason,contact_name,mobile,email,status) values(${ref},${conversationId}::uuid,${reason},${s.contactName ?? s.requesterName ?? s.contactPerson ?? null},${s.mobile ?? s.mobileNumber ?? s.requesterMobile ?? null},${s.email ?? s.requesterEmail ?? null},'new') returning id`.execute(
      this.db,
    );
    ...
    return {
      content:
        options?.content ??
        `I have passed this to the Tawseelhub team. Reference: ${ref}.\nDo you have another question, or how can I help you now?`,
      intent: "handoff" as const,
      status: "handed_off",
      structured: {
        state: this.completedWorkflowState(state),
  ...
  private async previousRequestQuestionResponse(
    conversationId: string,
    text: string,
    language: AgentLanguage,
    state: AgentState,
    ...
    const normalizedMobile =
      identity.mobileNormalized ??
      this.normalizeUaeMobile(
        String(state.slots.mobile ?? state.slots.mobileNumber ?? state.slots.requesterMobile ?? ""),
      );
    ...
    const latest = await this.latestPermittedCustomerObject(
      conversationId,
      normalizedMobile,
      referenceMatch,
    );
    ...
    return {
      ...
      structured: {
        state: {
          ...state,
          historicalContext: {
            latestReference: latest.reference_number,
  ...
  private async workflowInterruptionResponse(
    conversationId: string,
    text: string,
    language: AgentLanguage,
    previous: AgentState,
    ...
    if (previous.returningRequestDecision === "pending" && previous.existingRequest) {
      if (this.isNewShipmentDecision(text)) {
        ...
        const {
          ...
          lastAskedSlot: _lastAskedSlot,
          pendingAction: _pendingAction,
          returningRequestDecision: _returningRequestDecision,
          ...baseState
        } = current;
      ...
      if (this.isContinueExistingDecision(text) || this.isPriceQuestion(text)) {
        const quote = await this.quoteStatusForReference(
          conversationId,
          previous.existingRequest.reference,
          this.identityFromState(previous).mobileNormalized,
        );
        const content = this.existingQuoteDecisionMessage(
          quote ?? previous.existingRequest,
          language,
          {
            answerPrice: this.isPriceQuestion(text),
            continueOnly: this.isContinueExistingDecision(text) && !this.isPriceQuestion(text),
          },
        );
        ...
        return {
          ...
          structured: {
            ...
            state: {
              ...baseState,
              returningRequestDecision: "pending" as const,
              existingRequest: previous.existingRequest,
            },
          },
        };
      }
    ...
    if (this.isPriceQuestion(text)) {
        ...
        return {
          ...
          structured: {
            ...
            state: {
              ...
              returningRequestDecision: "pending" as const,
              existingRequest: {
                reference: String(active.reference_number),
      ...
      if (
        current.lastBusinessIntent === "customer_quote" ||
        previous.lastBusinessIntent === "customer_quote" ||
        ...
        return {
          ...
          intent: "customer_quote" as const,
          status: "waiting_for_user",
          structured: { suppressReturningAcknowledgement: true, state: current },
        };
      }
    }
    ...
    if (after && before !== after && current.lastBusinessIntent === "customer_quote") {
      ...
      if (active) {
        ...
        return {
          ...
          intent: "customer_quote" as const,
          status: "waiting_for_user",
          structured: { suppressReturningAcknowledgement: true, state },
        };
      }
    }
  ...
  private async returningCustomerAcknowledgement(
    conversationId: string,
    language: AgentLanguage,
    previous: AgentState,
    current: AgentState,
  ...
  private async quoteStatusForReference(
    conversationId: string,
  ...
  private existingQuoteDecisionMessage(
    quote: Record<string, unknown>,
    language: AgentLanguage,
    options: { answerPrice?: boolean; continueOnly?: boolean } = {},
  ) {
    const referenceNumber = String(quote.reference_number ?? quote.reference ?? "");
    const status = String(quote.status ?? "");
    const currency = String(quote.quote_currency ?? "AED");
    const minimumOffer =
      quote.minimum_offer === undefined || quote.minimum_offer === null
        ? null
        : Number(quote.minimum_offer);
  ...
  private async currentWorkflowPriceAnswer(
    state: AgentState,
    language: AgentLanguage,
  ): Promise<string> {
    ...
    if (missing.length) {
      const missingText =
        language === "ar"
          ? this.arabicMissingPriceFields(missing)
          : missing.map((field) => field.replace(/([A-Z])/g, " $1").toLowerCase()).join(", ");
      const base =
        language === "ar"
  ...
  private async estimateCurrentQuotePrice(
    slots: AgentSlots,
  ): Promise<{ currency: string; minimumOffer: number | null }> {
    const settings = (
      await sql<{
        enabled: boolean;
        commission_rate: string;
      }>`select enabled,commission_rate::text from platform_customer_marketplace_settings where id=true`.execute(
        this.db,
      )
    ).rows[0];
    ...
    const rules = (
      ...
      where cp.participates=true and cp.accepts_instant=true and p.status='active' and r.active=true and p.service_type=${String(slots.requestedServiceType ?? "standard")}`.execute(
        this.db,
      )
    ).rows;
  ...
  private async latestPermittedCustomerObject(
    conversationId: string,
    normalizedMobile?: string | null,
  ...
  private customerStatusLabel(status: string, language: AgentLanguage): string {
    ...
    return (
      (language === "ar" ? arabic : english)[status] ??
      (language === "ar" ? "قيد المتابعة" : "currently being followed up")
    );
  }
  ...
  private async answerGeneralQuestion(
    ...
    const generated =
      currentTurnIntent === "product_feature_question" ||
      currentTurnIntent === "current_feature_status" ||
      privateDirectoryOrCustomerInfo.test(latest) ||
      isAgentAnyPricingTopicText(latest)
        ? fallback()
    ...
    const content = this.enforceSingleQuestion(
      this.guardPublicReply(generated, language, fallback),
    );
    const shouldRememberFollowUp =
      /[?؟]/.test(content) &&
      (currentTurnIntent === "general_question" ||
        currentTurnIntent === "product_feature_question" ||
        currentTurnIntent === "current_feature_status");
    ...
    return {
      ...
      status: "waiting_for_user",
      structured: {
        state: shouldRememberFollowUp
          ? { ...baseState, pendingGeneralFollowUp: "public_explanation" }
          : baseState,
      },
    };
  }
  ...
  public async adminConversations(query: Record<string, string | undefined> = {}) {
    ...
    if (query.visibility === "hidden")
      filters.push(sql`c.hidden_at is not null and c.deleted_at is null`);
    ...
    if (query.assignedToAccountId === "unassigned")
      ...
      filters.push(sql`c.assigned_to_account_id=${query.assignedToAccountId}::uuid`);
    ...
    if (query.unread === "unread")
      filters.push(
        sql`exists(select 1 from platform_agent_messages um where um.conversation_id=c.id and um.sender_type='user' and um.created_at > coalesce(c.platform_last_read_at,'epoch'::timestamptz))`,
      );
    if (query.unread === "read")
      filters.push(
        sql`not exists(select 1 from platform_agent_messages um where um.conversation_id=c.id and um.sender_type='user' and um.created_at > coalesce(c.platform_last_read_at,'epoch'::timestamptz))`,
      );
    if (query.search?.trim()) {
      const search = `%${query.search.trim().toLowerCase()}%`;
      filters.push(
        sql`(lower(c.reference_number) like ${search} or lower(coalesce(c.customer_name,'')) like ${search} or coalesce(c.mobile_number_normalized,'') like ${search} or coalesce(c.mobile_number,'') like ${search} or lower(coalesce(q.reference_number,'')) like ${search} or lower(coalesce(t.reference_number,'')) like ${search} or lower(coalesce(d.reference_number,'')) like ${search})`,
      );
    }
  ...
  public async adminConversation(id: string) {
    ...
    await sql`update platform_agent_conversations set platform_last_read_at=now(),updated_at=now() where id = any(${ids}::uuid[])`.execute(
      this.db,
    );
    const conversation = (
      await sql<Record<string, unknown>>`
    ...
    const messages = await sql<
      Record<string, unknown>
    >`select m.id,m.sender_type,m.content,m.structured_payload,m.channel,m.direction,m.delivery_status,m.provider,m.provider_message_id,m.media_type,m.failure_code,m.created_at,c.reference_number conversation_reference from platform_agent_messages m join platform_agent_conversations c on c.id=m.conversation_id where m.conversation_id = any(${ids}::uuid[]) order by m.created_at,m.id`.execute(
      this.db,
    );
    const actions = await sql<
      Record<string, unknown>
    >`select a.action_type,a.status,a.request_snapshot,a.response_snapshot,a.safe_error_code,a.created_at,c.reference_number conversation_reference from platform_agent_actions a join platform_agent_conversations c on c.id=a.conversation_id where a.conversation_id = any(${ids}::uuid[]) order by a.created_at`.execute(
      this.db,
    );
    const comments = await sql<
      Record<string, unknown>
    >`select cc.id,cc.comment,cc.created_at,cc.updated_at,a.username author_username,c.reference_number conversation_reference from platform_agent_conversation_comments cc left join accounts a on a.id=cc.author_account_id join platform_agent_conversations c on c.id=cc.conversation_id where cc.conversation_id = any(${ids}::uuid[]) order by cc.created_at`.execute(
      this.db,
    );
    const history = await sql<
      Record<string, unknown>
    >`select h.*,actor.username actor_username,old_assignee.username old_assignee_username,new_assignee.username new_assignee_username,c.reference_number conversation_reference from platform_agent_conversation_status_history h left join accounts actor on actor.id=h.actor_account_id left join accounts old_assignee on old_assignee.id=h.old_assigned_to_account_id left join accounts new_assignee on new_assignee.id=h.new_assigned_to_account_id join platform_agent_conversations c on c.id=h.conversation_id where h.conversation_id = any(${ids}::uuid[]) order by h.created_at`.execute(
      this.db,
    );
    ...
    return {
      ...
      actions: actions.rows.map(mapRow),
      comments: comments.rows.map(mapRow),
      history: history.rows.map(mapRow),
      previousDays,
    };
  }

  public async updateConversationReview(
    id: string,
    input: AgentConversationReviewDto,
    actorId: string,
  ) {
    const ids = await this.dailyThreadConversationIds(id);
    const before = await sql<
      Record<string, unknown>
    >`select id,review_status,assigned_to_account_id from platform_agent_conversations where id = any(${ids}::uuid[])`.execute(
      this.db,
    );
    await sql`
      update platform_agent_conversations
  ...
  public async addConversationComment(id: string, comment: string, actorId: string) {
    const ids = await this.dailyThreadConversationIds(id);
    await sql`insert into platform_agent_conversation_comments(conversation_id,author_account_id,comment) values(${ids[0]}::uuid,${actorId}::uuid,${comment.trim()})`.execute(
      this.db,
    );
    return this.adminConversation(ids[0]!);
  }
  ...
  public async replyToWhatsAppConversation(id: string, message: string, actorId: string) {
    ...
    const conversation = (
      await sql<
        Record<string, unknown>
      >`select * from platform_agent_conversations where id=${ids[0]}::uuid`.execute(this.db)
    ).rows[0];
    if (!conversation) throw new NotFoundException();
    ...
    await sql`update platform_agent_messages set provider_message_id=${send.providerMessageId},delivery_status=${send.status},failure_code=${send.failureCode ?? null} where id=${messageId}::uuid`.execute(
      this.db,
    );
    await sql`update platform_agent_settings set whatsapp_last_outbound_at=case when ${send.status}<>'failed' then now() else whatsapp_last_outbound_at end, whatsapp_last_error_code=${send.failureCode ?? null} where id=true`.execute(
      this.db,
    );
    await sql`insert into platform_agent_conversation_status_history(conversation_id,old_status,new_status,actor_account_id,comment) values(${ids[0]}::uuid,${String(conversation.review_status ?? "open")},${String(conversation.review_status ?? "open")},${actorId}::uuid,'Platform WhatsApp reply sent')`.execute(
      this.db,
    );
    return this.adminConversation(ids[0]!);
  }
  ...
  public async replyToWebsiteConversation(id: string, message: string, actorId: string) {
    ...
    if (!conversation) throw new NotFoundException();
    if (conversation.channel !== "website" && conversation.last_channel !== "website")
      throw new BadRequestException("conversation_is_not_website_chat");
    if (conversation.conversation_mode !== "human_active")
      throw new BadRequestException("human_takeover_required");
    const assignedTo =
      conversation.assigned_to_account_id === null ||
      conversation.assigned_to_account_id === undefined
        ? null
        : String(conversation.assigned_to_account_id);
    if (assignedTo && assignedTo !== actorId)
      throw new BadRequestException("conversation_owned_by_another_agent");
    await this.appendMessage(
      ids[0]!,
      "platform_staff",
      message.trim(),
      { publicLabel: "Tawseelhub Team" },
      {
        channel: "website",
    ...
    await sql`insert into platform_agent_conversation_status_history(conversation_id,old_status,new_status,actor_account_id,comment) values(${ids[0]}::uuid,${String(conversation.review_status ?? "open")},${String(conversation.review_status ?? "open")},${actorId}::uuid,'Platform website reply sent')`.execute(
      this.db,
    );
    return this.adminConversation(ids[0]!);
  }

  public async setConversationMode(
    id: string,
    mode: "ai_active" | "human_active" | "paused" | "ai_resume",
    actorId: string,
    note?: string,
  ) {
    const ids = await this.dailyThreadConversationIds(id);
    const before = await sql<
      Record<string, unknown>
    >`select id,conversation_mode,review_status,assigned_to_account_id from platform_agent_conversations where id = any(${ids}::uuid[])`.execute(
      this.db,
    );
    ...
    if (
      mode === "human_active" &&
      before.rows.some(
        (row) => row.assigned_to_account_id && String(row.assigned_to_account_id) !== actorId,
      )
    )
      throw new BadRequestException("conversation_owned_by_another_agent");
    await sql`
    ...
    for (const row of before.rows) {
      await this.recordModeHistory(
        String(row.id),
        String(row.conversation_mode ?? "ai_active"),
        mode,
        actorId,
        note ??
          (mode === "human_active"
  ...
  public async platformAssignees() {
    const rows = await sql<
      Record<string, unknown>
    >`select id,username,status from accounts where company_id is null and account_kind='platform_administrator' order by username`.execute(
      this.db,
    );
  ...
  public async hideConversation(id: string, actorId: string) {
    ...
    if (!conversation) throw new NotFoundException();
    await sql`insert into platform_agent_conversation_status_history(conversation_id,old_status,new_status,actor_account_id,comment) values(${id}::uuid,${String(conversation.review_status ?? "open")},${String(conversation.review_status ?? "open")},${actorId}::uuid,'Conversation hidden from Agent inbox')`.execute(
      this.db,
    );
  ...
  public async unhideConversation(id: string, actorId: string) {
    ...
    if (!conversation) throw new NotFoundException();
    await sql`insert into platform_agent_conversation_status_history(conversation_id,old_status,new_status,actor_account_id,comment) values(${id}::uuid,${String(conversation.review_status ?? "open")},${String(conversation.review_status ?? "open")},${actorId}::uuid,'Conversation restored to Agent inbox')`.execute(
      this.db,
    );
  ...
  public async deleteConversation(id: string, actorId: string) {
    ...
    await this.db.transaction().execute(async (trx) => {
      ...
      if (handoffIds.length > 0) {
        await sql`alter table platform_agent_handoff_history disable trigger agent_handoff_history_append_only`.execute(
          trx,
        );
        await sql`
        ...
        `.execute(trx);
        await sql`alter table platform_agent_handoff_history enable trigger agent_handoff_history_append_only`.execute(
          trx,
        );
        await sql`
      ...
      await sql`delete from platform_agent_conversation_status_history where conversation_id = any(${ids}::uuid[])`.execute(
        trx,
      );
      await sql`delete from platform_agent_conversation_comments where conversation_id = any(${ids}::uuid[])`.execute(
        trx,
      );
      await sql`delete from platform_agent_actions where conversation_id = any(${ids}::uuid[])`.execute(
        trx,
      );
      await sql`delete from platform_agent_messages where conversation_id = any(${ids}::uuid[])`.execute(
        trx,
      );
  ...
  public async handoffs() {
    const rows = await sql<
      Record<string, unknown>
    >`select h.*,c.reference_number conversation_reference,c.channel from platform_agent_handoffs h join platform_agent_conversations c on c.id=h.conversation_id order by h.created_at desc limit 200`.execute(
      this.db,
    );
    return rows.rows.map(mapRow);
  }
  ...
  public async updateHandoffStatus(
    ...
    const before = (
      await sql<{
        status: string;
      }>`select status from platform_agent_handoffs where id=${id}::uuid`.execute(this.db)
    ).rows[0];
    if (!before) throw new NotFoundException();
    await sql`update platform_agent_handoffs set status=${status},updated_at=now() where id=${id}::uuid; insert into platform_agent_handoff_history(handoff_id,old_status,new_status,actor_account_id,notes) values(${id}::uuid,${before.status},${status},${actorId}::uuid,${notes ?? null})`.execute(
      this.db,
    );
  ...
  public async knowledge() {
    return (
      await sql<
        Record<string, unknown>
      >`select * from platform_agent_knowledge order by language,status,sort_order,title`.execute(
        this.db,
      )
    ).rows.map(mapRow);
  }
  ...
  public async saveKnowledge(input: AgentKnowledgeDto, actorId: string, id?: string) {
    ...
    if (visibility === "internal_only" && featureStatus !== "internal_only")
      throw new BadRequestException("internal_visibility_requires_internal_status");
    if (id)
      await sql`update platform_agent_knowledge set language=${input.language},title=${input.title},content=${input.content},category=${input.category},audience=${audience},feature_status=${featureStatus},visibility=${visibility},status=${input.status},sort_order=${input.sortOrder ?? 100},updated_by_account_id=${actorId}::uuid,updated_at=now() where id=${id}::uuid`.execute(
        this.db,
      );
    else
      await sql`insert into platform_agent_knowledge(language,title,content,category,audience,feature_status,visibility,status,sort_order,created_by_account_id,updated_by_account_id) values(${input.language},${input.title},${input.content},${input.category},${audience},${featureStatus},${visibility},${input.status},${input.sortOrder ?? 100},${actorId}::uuid,${actorId}::uuid)`.execute(
        this.db,
      );
  ...
  public async publicWhatsAppSettings() {
    ...
    const enabled =
      Boolean(row.whatsapp_public_cta_enabled) && row.whatsapp_provider !== "disabled";
    const number = String(row.whatsapp_business_number ?? "+971 50 689 8604");
    const normalized =
      normalizeInternationalMobile(String(row.whatsapp_business_number_normalized ?? number)) ??
      "971506898604";
    return {
      enabled,
  ...
  private async recordWebhook(
    ...
    senderMobileNormalized: string | null,
  ...
  private inboxDateRange(preset?: string, from?: string, to?: string): { from?: Date; to?: Date } {
    ...
    const startOfDay = (date: Date) =>
      new Date(Date.UTC(date.getFullYear(), date.getMonth(), date.getDate()) - 4 * 60 * 60 * 1000);
    if (from || to) {
      return {
        ...(from ? { from: startOfDay(new Date(`${from}T00:00:00`)) } : {}),
        ...(to
          ? { to: new Date(startOfDay(new Date(`${to}T00:00:00`)).getTime() + 24 * 60 * 60 * 1000) }
          : {}),
      };
    }
    if (preset === "today")
      return {
        from: startOfDay(dubaiNow),
        to: new Date(startOfDay(dubaiNow).getTime() + 24 * 60 * 60 * 1000),
      };
    if (preset === "yesterday")
      return {
        from: new Date(startOfDay(dubaiNow).getTime() - 24 * 60 * 60 * 1000),
        to: startOfDay(dubaiNow),
      };
    if (preset === "this_week") {
  ...
  private classificationFor(intent: AgentIntent, state: AgentState): string {
    if (intent === "customer_quote" || state.pendingAction?.type === "calculate_customer_quote")
      return "shipment_quote";
    if (intent === "trader" || state.pendingAction?.type === "submit_trader_application")
      return "trader_lead";
    ...
    if (topics.includes("storefront") || topics.includes("integrations"))
      return "storefront_commerce";
    if (topics.includes("customer_quotes")) return "pricing_enquiry";
    if (intent === "product_feature_question" || intent === "current_feature_status")
      return "product_question";
    if (state.audience === "delivery_company" && topics.includes("delivery_companies"))
      return "delivery_company_lead";
  ...
  private askForSlot(field: keyof AgentSlots, language: AgentLanguage): string {
    const en: Partial<Record<keyof AgentSlots, string>> = {
      deliveryAddress:
        "What delivery address or landmark should I use? You can type “skip” if you do not have it now.",
    ...
    return (
      (language === "ar" ? ar[field] : en[field]) ??
      (language === "ar" ? "ما المعلومة التي أستخدمها هنا؟" : "What should I use for that detail?")
    );
  }
  ...
  private identityFromState(state: AgentState) {
    const name =
      state.slots.requesterName ??
      state.slots.contactPerson ??
      state.slots.contactName ??
      state.slots.storeName ??
      state.slots.companyName ??
      null;
    const mobileOriginal =
      state.slots.requesterMobile ?? state.slots.mobileNumber ?? state.slots.mobile ?? null;
  ...
  private mergeSlots(current: AgentSlots, extracted: AgentSlots, correction: boolean): AgentSlots {
    ...
    if (next.pickupBusinessArea === undefined && next.pickupArea)
      next.pickupBusinessArea = next.pickupArea;
    return next;
  }

  private applySequentialWorkflowAnswer(
    text: string,
    intent: AgentIntent,
    previousState: AgentState,
    slots: AgentSlots,
    ...
    if (!previousMissing || (slots[previousMissing] !== undefined && slots[previousMissing] !== ""))
      return slots;
    if (
      previousMissing === "pickupEmirate" ||
      previousMissing === "deliveryEmirate" ||
      previousMissing === "emirate"
    ) {
      ...
      if (previousMissing === "deliveryEmirate") {
        ...
        if (previousState.slots.pickupEmirate)
          next.pickupEmirate = previousState.slots.pickupEmirate;
      }
      return next;
    }
    ...
    if (previousMissing === "pickupArea")
      return {
        ...slots,
        pickupArea: answer,
        pickupBusinessArea: slots.pickupBusinessArea ?? answer,
      };
    ...
    if (previousMissing === "requesterMobile")
      return { ...slots, requesterMobile: answer, mobileNumber: slots.mobileNumber ?? answer };
    if (previousMissing === "deliveryAddress")
      return this.isSkipAnswer(answer) ? slots : { ...slots, deliveryAddress: answer };
    ...
    if (previousMissing === "contactPerson") {
      if (/\b(i sell|we sell|need delivery|orders?|instagram|shopify|store|trader)\b/i.test(answer))
        return slots;
      return { ...slots, contactPerson: answer, contactName: slots.contactName ?? answer };
    }
    if (previousMissing === "contactName")
      return { ...slots, contactName: answer, requesterName: slots.requesterName ?? answer };
    if (previousMissing === "mobile")
      return {
        ...slots,
        mobile: answer,
        mobileNumber: slots.mobileNumber ?? answer,
        requesterMobile: slots.requesterMobile ?? answer,
      };
    if (previousMissing === "mobileNumber")
      return { ...slots, mobileNumber: answer, requesterMobile: slots.requesterMobile ?? answer };
    if (previousMissing === "email")
      return { ...slots, email: answer, requesterEmail: slots.requesterEmail ?? answer };
    if (previousMissing === "pickupBusinessArea")
      return { ...slots, pickupBusinessArea: answer, pickupArea: slots.pickupArea ?? answer };
    if (previousMissing === "trackingAirwayBill") return { ...slots, trackingAirwayBill: answer };
    if (previousMissing === "trackingMobileNumber")
      return { ...slots, trackingMobileNumber: answer };
    return slots;
  }

  private resolveTurnIntent(
    text: string,
    ...
    if (
      !state.lastAskedSlot ||
      !state.lastBusinessIntent ||
      !workflowIntents.has(state.lastBusinessIntent)
    )
      return classifiedIntent;
    if (this.isPriceQuestion(text)) return classifiedIntent;
    if (
      classifiedIntent === "handoff" ||
      classifiedIntent === "current_feature_status" ||
      classifiedIntent === "product_feature_question"
    )
      return classifiedIntent;
    if (
      this.isClarification(text) ||
      this.isWorkflowCancellation(text) ||
      this.looksLikeNewBusinessQuestion(text)
    )
      return classifiedIntent;
    const answer = text.trim();
    if (!answer || answer.length > 120 || /[?؟]/.test(answer)) return classifiedIntent;
    if (
      genericAnswerIntents.has(classifiedIntent) ||
      socialIntents.has(classifiedIntent) ||
      workflowIntents.has(classifiedIntent)
    )
      return state.lastBusinessIntent;
    return classifiedIntent;
  }

  private intentFromConversationFrame(
    frame: AgentState["conversationFrame"],
    classifiedIntent: AgentIntent,
  ): AgentIntent {
    if (!frame) return classifiedIntent;
    if (
      frame.decision === "explicit_workflow_start" ||
      frame.decision === "workflow_continue" ||
      frame.decision === "current_workflow_slot_response"
    ) {
    ...
    if (frame.mode === "conversation" && frame.workflowState !== "active") {
      if (
        ...
        return "product_feature_question";
  ...
  private stateScopedByConversationFrame(
    state: AgentState,
    frame: AgentState["conversationFrame"],
  ): AgentState {
    if (!frame) return state;
    if (
      frame.decision === "bare_topic_information" ||
      frame.decision === "workflow_paused_for_explanation" ||
      frame.decision === "workflow_cancelled" ||
      frame.decision === "informational_topic" ||
      frame.decision === "privacy_blocked"
    ) {
  ...
  private logFrameDecision(
    conversationId: string,
    frame: NonNullable<AgentState["conversationFrame"]>,
  ): void {
    this.logger.debug(
      {
        conversationId,
  ...
  private previousIntentForClassification(
  ...
  private isWorkflowCancellation(text: string): boolean {
    return /^(cancel|stop|never mind|start over|reset|الغاء|إلغاء|وقف|خلاص|ابدأ من جديد)$/i.test(
      text.trim(),
    );
  }
  ...
  private looksLikeNewBusinessQuestion(text: string): boolean {
    ...
    return /^(can|do|does|is|are|how|what|which|when|where|why|هل|كيف|شو|ما|متى|وين)\b/i.test(
      lower,
    );
  }
  ...
  private isClarification(text: string): boolean {
    return (
      /^(why|why\?|for what|what for|for pickup or delivery|pickup or delivery|is this pickup or delivery|what do you mean|is it required|why do you need this|why do you need .*address)\??$/i.test(
        text.trim(),
      ) ||
      /^(ليش|لماذا|لشو|حق شو|شو تقصد|عنوان الاستلام ولا التوصيل|استلام ولا توصيل|للاستلام ولا للتوصيل|هل هو مطلوب|هل مطلوب)[؟?]?$/.test(
        text.trim(),
      ) ||
      /ليش.*العنوان|ليش.*عنوان|لماذا.*العنوان|عنوان.*(?:استلام|توصيل)/i.test(text.trim())
    );
  }

  private fieldClarification(
    field: keyof AgentSlots,
    language: AgentLanguage,
    state: AgentState,
    text: string,
  ) {
    if (field === "deliveryAddress") {
      const pickup = [
        state.slots.pickupArea,
        this.publicEmirateLabel(state.slots.pickupEmirate, language),
      ]
        .filter(Boolean)
        .join(", ");
      const delivery = [
        state.slots.deliveryArea,
        this.publicEmirateLabel(state.slots.deliveryEmirate, language),
      ]
        .filter(Boolean)
        .join(", ");
      const asksPickupOrDelivery =
        /pickup or delivery|(?:ال)?استلام ولا (?:ال)?توصيل|للاستلام ولا للتوصيل/i.test(text);
      const asksForWhat = /for what|what for|لشو|حق شو/i.test(text);
      if (language === "ar") {
        const context =
          pickup && delivery ? `الاستلام محفوظ عندي: ${pickup}. والتوصيل إلى ${delivery}.` : "";
        ...
        return {
          content: `${explanation}\nما عنوان التوصيل أو أقرب معلم؟ وإذا غير متوفر الآن اكتب “تخطي”.`,
      ...
      const context =
        pickup && delivery
          ? `I already have your pickup as ${pickup}, and delivery as ${delivery}.`
          : "";
      const explanation = asksPickupOrDelivery
    ...
    const en: Partial<Record<keyof AgentSlots, string>> = {
      deliveryAddress:
        "The delivery address helps the Delivery Company estimate the route and quotation more accurately. If you do not have the exact address yet, you can give the closest area or landmark.",
      pickupArea:
        "The pickup area helps estimate the route and match the right delivery option. The Emirate alone is usually not enough for an accurate quotation.",
      deliveryArea:
        "The delivery area helps estimate distance and service availability. The Emirate alone is usually not enough for an accurate quotation.",
      requesterName:
        "We ask for the name so the Tawseelhub team can identify the quote request and follow up correctly.",
      requesterMobile:
        "We ask for a UAE mobile number so Tawseelhub can contact you with the quotation or any needed clarification.",
    };
    const ar: Partial<Record<keyof AgentSlots, string>> = {
      deliveryAddress:
        "العنوان يساعد شركة التوصيل على تقدير المسار والسعر بشكل أدق. إذا ما عندك العنوان الكامل الآن، ممكن تكتب أقرب منطقة أو معلم واضح.",
      pickupArea:
        "منطقة الاستلام تساعد في تقدير المسار واختيار عرض التوصيل المناسب. الإمارة وحدها غالباً لا تكفي لسعر دقيق.",
      deliveryArea:
        "منطقة التوصيل تساعد في تقدير المسافة وتوفر الخدمة. الإمارة وحدها غالباً لا تكفي لسعر دقيق.",
      requesterName:
        "نطلب الاسم حتى يتمكن فريق Tawseelhub من تمييز طلب السعر والمتابعة معك بشكل صحيح.",
      requesterMobile:
        "نطلب رقم هاتف إماراتي حتى يتواصل معك فريق Tawseelhub بعرض السعر أو أي توضيح مطلوب.",
    };
    return {
      content:
        (language === "ar" ? ar[field] : en[field]) ??
        (language === "ar"
          ? "أحتاج هذه المعلومة حتى أتابع الطلب بشكل صحيح. إذا غير متوفرة الآن، أخبرني بما هو متاح."
          : "I need that detail to continue the request correctly. If you do not have it now, tell me what you do have."),
      intent: "clarification" as const,
      status: "waiting_for_user",
    };
  }
  ...
  private state(value: unknown): AgentState {
    ...
    const state: AgentState = {
      audience:
        object.audience === "delivery_company" ||
        object.audience === "trader" ||
        object.audience === "customer"
          ? object.audience
          : "unknown",
      discussedTopics: Array.isArray(object.discussedTopics)
        ? object.discussedTopics.filter((item): item is string => typeof item === "string")
        : [],
      slots: asObject(object.slots) as AgentSlots,
      seenInboundMessageIds: Array.isArray(object.seenInboundMessageIds)
        ? (object.seenInboundMessageIds as string[])
        : [],
    };
    if (typeof object.lastBusinessIntent === "string")
      state.lastBusinessIntent = object.lastBusinessIntent as AgentIntent;
    if (typeof object.lastAskedSlot === "string")
      state.lastAskedSlot = object.lastAskedSlot as keyof AgentSlots;
    ...
    if (object.existingRequest !== undefined) {
      ...
      if (typeof existing.reference === "string" && typeof existing.status === "string")
        state.existingRequest = { reference: existing.reference, status: existing.status };
    }
    if (object.deliveryAddressSkipped === true) state.deliveryAddressSkipped = true;
    if (
      object.pendingGeneralFollowUp === "public_explanation" ||
      object.pendingGeneralFollowUp === "feature_choice" ||
      object.pendingGeneralFollowUp === "trader_registration_explained"
    )
      state.pendingGeneralFollowUp = object.pendingGeneralFollowUp;
    if (object.historicalContext !== undefined)
      state.historicalContext = asObject(object.historicalContext) as NonNullable<
        AgentState["historicalContext"]
      >;
    if (object.pendingAction !== undefined) {
      state.pendingAction = object.pendingAction as NonNullable<AgentState["pendingAction"]>;
    }
    if (object.conversationFrame !== undefined)
      state.conversationFrame = asObject(object.conversationFrame) as unknown as NonNullable<
        AgentState["conversationFrame"]
      >;
    if (object.tracking !== undefined)
      state.tracking = asObject(object.tracking) as unknown as NonNullable<AgentState["tracking"]>;
    return state;
  }
  ...
  private async findByToken(sessionToken: string) {
    const row = (
      await sql<
        Record<string, unknown>
      >`select * from platform_agent_conversations where public_session_token_hash=${hash(sessionToken)}`.execute(
        this.db,
      )
    ).rows[0];
    ...
    return {
      ...
      messages: messages.rows.map((row) => {
        ...
        return {
          ...mapped,
          displayName:
            row.sender_type === "platform_staff"
              ? "Tawseelhub Team"
  ...
  private humanUnavailableContactState(state: AgentState): AgentState {
    ...
    return {
      ...
      lastAskedSlot:
        (slots.contactName ?? slots.requesterName ?? slots.contactPerson)
          ? "mobile"
          : "contactName",
    };
  }

  private async markWebsiteHumanRequested(
    conversationId: string,
    oldMode: string,
    comment: string,
  ) {
  ...
  private async recordModeHistory(
    conversationId: string,
    oldMode: string,
    newMode: string,
    actorId: string | null,
  ...
  private async appendMessageByReference(
    ref: string,
    sender: string,
    content: string,
    payload?: unknown,
  ) {
    await sql`insert into platform_agent_messages(conversation_id,sender_type,content,structured_payload,channel,direction) select id,${sender},${content},${payload === undefined ? null : JSON.stringify(payload)}::jsonb,channel,case when ${sender}='user' then 'inbound' when ${sender}='system' then 'internal' else 'outbound' end from platform_agent_conversations where reference_number=${ref}`.execute(
      this.db,
    );
    await sql`update platform_agent_conversations set last_message_at=now(),updated_at=now() where reference_number=${ref}`.execute(
      this.db,
    );
  }
  ...
  private socialResponse(intent: AgentIntent, language: AgentLanguage) {
    ...
    const content =
      language === "ar"
        ? ({
            greeting:
              "مرحباً! كيف يمكنني مساعدتك اليوم؟ يمكنك السؤال عن Tawseelhub أو إرسال شحنة أو تسجيل تاجر أو طلب عرض لشركة توصيل.",
        ...
        : ({
            greeting:
              "Hi! How can I help you today? You can ask about Tawseelhub, send a package, register as a Trader, or request a Delivery Company demo.",
  ...
  private enrichState(text: string, intent: AgentIntent, state: AgentState): AgentState {
    ...
    return {
      ...
      ...(businessInfoIntents.has(intent) ||
      intent === "customer_quote" ||
      intent === "trader" ||
      intent === "delivery_company_demo"
        ? { lastBusinessIntent: intent }
        : {}),
    };
  }
  ...
  private detectAudience(text: string, state: AgentState): NonNullable<AgentState["audience"]> {
    const lower = text.toLowerCase();
    if (/delivery company|courier company|fleet|drivers|driver|شركة توصيل|سائق/.test(lower))
      return "delivery_company";
    if (/trader|store|seller|shopify|salla|woocommerce|instagram|تاجر|متجر/.test(lower))
      return "trader";
    if (
      /send|shipment|package|parcel|quote|pickup|deliver|delivery from|kg|box|personal package|customer|أرسل|ارسل|إرسال|ارسال|شحنة|طرد|سعر|طلب توصيل|أحتاج توصيل|احتاج توصيل/.test(
        lower,
      )
    )
      return "customer";
  ...
  private async latestUserMessage(conversationId: string): Promise<string> {
    const row = (
      await sql<{
        content: string;
      }>`select content from platform_agent_messages where conversation_id=${conversationId}::uuid and sender_type='user' order by created_at desc limit 1`.execute(
        this.db,
      )
    ).rows[0];
    return row?.content ?? "";
  ...
  private async retrieveKnowledge(
    ...
    const topics = new Set([
      ...this.detectTopics(text, state.lastBusinessIntent ?? "unknown"),
      ...(state.discussedTopics ?? []),
    ]);
    const scored = rows.rows
      .map((row) => {
        ...
        for (const topic of topics)
          if (searchable.includes(topic.replace(/_/g, " ")) || searchable.includes(topic))
            score += 4;
    ...
    return selected.map(({ row }) => ({
      ...
      featureStatus: String(
        row.feature_status ?? "informational",
      ) as AgentKnowledgeContext["featureStatus"],
      title: String(row.title ?? ""),
      visibility: String(row.visibility ?? "public_agent") as AgentKnowledgeContext["visibility"],
    }));
  }

  private safeBusinessFallback(
    text: string,
    language: AgentLanguage,
    state: AgentState,
    ...
    if (/reconciliation|driver collections|driver money/.test(lower))
      return language === "ar"
    ...
    if (language === "ar") {
      ...
      if (isAgentTraderUsageQuestionText(text))
        return "يستخدم التاجر Tawseelhub لربط متجره أو طلباته مع عمليات التوصيل: تسجيل بيانات المتجر، تحديد موقع الاستلام، متابعة الطلبات، رؤية حالة التوصيل، ومتابعة التحصيل COD والتسويات حسب إعداد شركة التوصيل. التسجيل كتاجر هو الخطوة الأولى إذا كان يريد من فريق Tawseelhub مراجعة بياناته وربطه بخدمة توصيل مناسبة. هل تريد شرح التسجيل، أم متابعة الطلبات بعد التسجيل؟";
      if (/shopify|شوبيفاي/.test(lower))
        return "تكامل Shopify مخطط وليس متاحاً حالياً. الهدف أن يحتفظ التاجر بمتجر Shopify بينما تنتقل طلبات التوصيل إلى Tawseelhub عند توفر التكامل.";
      if (/محاسبة|حسابات/.test(lower))
        return "نعم، يوجد دعم محاسبي تشغيلي داخل Tawseelhub لمتابعة COD والتحصيل والتسويات والتقارير المالية المرتبطة بعمليات التوصيل.";
      if (/رواتب|راتب|Payroll/i.test(lower))
        return "نعم، يوجد دعم للرواتب/مستحقات السائقين والموظفين حسب إعدادات الشركة، وليس كمنتج رواتب عام منفصل.";
      if (/تطبيق|موبايل|mobile/i.test(lower))
        return "تدفقات تطبيقات الموبايل جزء من نموذج Tawseelhub للعمليات الميدانية وتجربة العملاء. حالة كل تطبيق أو وحدة تعتمد على إعدادات التشغيل المتاحة للشركة.";
      return (
        knowledge.find((item) => /[\u0600-\u06ff]/.test(item.content))?.content ??
        arabicGeneralFallback
      );
    }
    if (
      state.audience === "trader" &&
      /receive.*money|paid|payment|settlement|my money/.test(lower)
    )
      return "You receive your money through Trader settlement: delivered orders and COD amounts are tracked, then the Delivery Company/Tawseelhub process records what is payable to you and follows it through statement and settlement status. Exact timing depends on the commercial arrangement.";
    if (isAgentFeatureExplanationText(text)) return featureExplanationText(text, language);
    if (isAgentTraderUsageQuestionText(text))
      return "A Trader uses Tawseelhub to connect store/order details with delivery operations: store registration, pickup location, order follow-up, delivery status, COD tracking and settlement follow-up depending on the Delivery Company setup. Trader registration is the first step if they want Tawseelhub to review their details and connect them to a suitable delivery service. Would you like me to explain registration or order follow-up after registration?";
    if (/manage .*drivers|manage my drivers|drivers|driver management|fleet/.test(lower))
      return "Yes. Tawseelhub supports Delivery Company driver operations such as keeping driver records, assigning orders, following delivery progress, and connecting driver activity to collections and reports. If you have 20 drivers, the main value is seeing orders, drivers and COD in one operating view.";
    if (/accounting|journal|financial/.test(lower))
      return "Yes. Tawseelhub includes accounting-oriented records and reporting for delivery operations, so operational activity such as COD, collections and settlements can be followed more clearly. It is part of the operating system, not a separate public accounting product.";
    if (/payroll|salary|earning/.test(lower))
      return "Yes. Payroll support is included for driver/employee compensation workflows in the delivery operating system. It should be used with the configured company rules rather than treated as a public standalone payroll app.";
    if (/manage traders|trader relationship|traders/.test(lower))
      return "Yes. Delivery Companies can manage Trader relationships in Tawseelhub, including Trader profiles, orders, COD balances, statements and settlement follow-up. I won’t expose private Trader names publicly, but the platform can organize your own Trader base.";
    if (/report|reports|statement/.test(lower))
      return "Tawseelhub provides operational reports around orders, drivers, COD collections, Trader statements, settlements and delivery performance. The exact report set depends on what your company has configured.";
    if (/mobile app|driver app|apps/.test(lower))
      return "Mobile app workflows are part of the Tawseelhub operating model for field and customer-facing use. Core delivery operations are live in the platform; specific mobile app modules should be treated as app-enabled workflows unless your Tawseelhub configuration confirms that module is live.";
    if (/shopify/.test(lower))
      return "Shopify integration is planned, not live yet. The goal is for Traders to keep Shopify while delivery orders flow into Tawseelhub and then to their Delivery Company. Are you asking as a Trader or a Delivery Company?";
    if (/salla/.test(lower))
      return "Salla integration is planned, not live yet. The goal is for Traders to keep their Salla store while delivery orders flow into Tawseelhub. Do you already operate a Salla store?";
    if (/woocommerce/.test(lower))
      return "WooCommerce integration is planned, not live yet. It is intended to connect store orders into Tawseelhub delivery workflows. Are you asking from the Trader side or Delivery Company side?";
    if (/storefront|build a store|create a store/.test(lower))
      return "A Tawseelhub Storefront concept is part of the roadmap, but it is currently on hold while Tawseelhub focuses on the Delivery Operating System, customer quote service, Trader onboarding and planned commerce integrations. Do you already have an online store today?";
    if (/customers.*shop|shop on tawseelhub|sell products through tawseelhub/.test(lower))
      return "Customer shopping through Tawseelhub is not live as a public marketplace. Trader commerce and Storefront capabilities exist as a foundation/roadmap area, while public customer shopping should not be described as live.";
    const base = knowledge[0]?.content ?? "I do not have confirmed information about that yet.";
    if (state.audience === "delivery_company")
      return `${base} Which part of your delivery operation are you trying to improve first?`;
    if (state.audience === "trader")
      return `${base} Are you already working with a Delivery Company, or are you looking for one?`;
  ...
  private guardPublicReply(text: string, language: AgentLanguage, fallback: () => string): string {
    const unsafe =
      /(api[_ -]?key|company[_ -]?id|commission|company net|net amount|marketplace priority|internal pricing|delivery company directory|show.*delivery companies|15\s*%)/i;
    ...
    if (language === "ar") {
      trimmed = trimmed
        .replace(
          /^(?:مرحباً|أهلاً|اهلاً|هلا|Hi|Hello)?\s*[—–-]?\s*أنا\s+(?:يوسف|يوسُف|Yousef)[،,\s-]*(?:مساعد\s+)?(?:Tawseelhub|توصيلهَب|توصيلهب)?(?:\s*(?:AI|بالذكاء الاصطناعي|الآلي|Assistant|مساعد))?[.،,\s-]*/i,
          "",
        )
        .replace(
          /^(?:مرحباً|أهلاً|اهلاً|هلا|Hi|Hello)\s*[—–-]\s*(?:يوسف|Yousef)\s*(?:هنا|here)?[.،,\s-]*/i,
          "",
        )
