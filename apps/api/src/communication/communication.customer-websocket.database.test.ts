...
describe.skipIf(!enabled)("guarded Customer communication WebSocket security", () => {
  ...
  it("authenticates a valid Customer socket, replays only its own conversation, denies cross-scope subscriptions, and disconnects on revocation", async () => {
  ...
  it("deduplicates replayed events already delivered live and never leaks another conversation's events", async () => {
    await withRolledBackCommunicationFixtures(database, async (transaction, runId) => {
      ...
      try {
        ...
        const deliveredSequences = new Set(
          (recoveryOne.events ?? []).map((event) => event.sequence),
        );
