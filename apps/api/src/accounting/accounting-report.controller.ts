...
export class AccountingReportController {
  ...
  public readiness() { return this.reports.readiness(); }

  @Get("documents/journals/:id/pdf")
  @RequireAnyPermission("accounting.view", "users_roles.manage")
  public journalPdf(@Param("id", ParseUUIDPipe) id: string, @Query() query: AccountingDocumentQueryDto, @Res() response: Response) {
    return this.sendDocument("journal", id, query, response);
  }

  @Get("documents/opening-balances/:id/pdf")
  @RequireAnyPermission("accounting.view", "users_roles.manage")
  public openingPdf(@Param("id", ParseUUIDPipe) id: string, @Query() query: AccountingDocumentQueryDto, @Res() response: Response) {
    return this.sendDocument("opening-balance", id, query, response);
  }

  @Get("documents/expenses/:id/pdf")
  @RequireAnyPermission("accounting.view", "users_roles.manage")
  public expensePdf(@Param("id", ParseUUIDPipe) id: string, @Query() query: AccountingDocumentQueryDto, @Res() response: Response) {
    return this.sendDocument("expense", id, query, response);
  }

  @Get("documents/expense-payments/:id/pdf")
  @RequireAnyPermission("accounting.view", "users_roles.manage")
  public paymentPdf(@Param("id", ParseUUIDPipe) id: string, @Query() query: AccountingDocumentQueryDto, @Res() response: Response) {
    return this.sendDocument("expense-payment", id, query, response);
  }

  @Get("documents/cash-bank-movements/:id/pdf")
  @RequireAnyPermission("accounting.view", "users_roles.manage")
  public movementPdf(@Param("id", ParseUUIDPipe) id: string, @Query() query: AccountingDocumentQueryDto, @Res() response: Response) {
  ...
  public async export(
    ...
    const filename = safeAccountingFilename(`${kind}-${output.report.snapshotAt.slice(0, 10)}.${output.extension}`);
  ...
  private kind(value: string): AccountingReportKind {
    if (!accountingReportKinds.includes(value as AccountingReportKind)) {
      throw new ApplicationException("accounting_report_not_found", "The requested Accounting report is not available", HttpStatus.NOT_FOUND);
    }
  ...
  private async sendDocument(
    ...
    response.setHeader("Content-Type", "application/pdf");
    response.setHeader("Content-Disposition", `${disposition}; filename="${safeAccountingFilename(`${type}-${document.reference}.pdf`)}"`);
