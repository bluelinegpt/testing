...
function zip(files: readonly { readonly name: string; readonly body: string }[]): Buffer {
  ...
  for (const file of files) {
    ...
    directory.writeUInt16LE(0, 10); directory.writeUInt32LE(crc, 16);
  ...
  end.writeUInt32LE(centralBytes.length, 12); end.writeUInt32LE(localBytes.length, 16);
...
export function accountingXlsx(
  ...
  const sheetRows = [
    ...
    `<row r="${index + 1}">${row.map((value) =>
      `<c t="inlineStr"><is><t xml:space="preserve">${xml(value)}</t></is></c>`).join("")}</row>`,
  ).join("");
  return zip([
    ...
    { name: "xl/workbook.xml", body: `<?xml version="1.0" encoding="UTF-8"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Report" sheetId="1" r:id="rId1"/></sheets></workbook>` },
