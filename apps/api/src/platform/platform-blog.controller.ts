...
export class PlatformBlogController {
  ...
  @RequirePlatformPermissions(READ) @Get("articles/:id/seo-readiness") readiness(@Param("id", new ParseUUIDPipe())id:string){return this.blog.seoReadiness(id);}
  ...
  @RequirePlatformPermissions(READ) @Get("articles/:id") detail(@Param("id", new ParseUUIDPipe()) id: string) {
    return this.blog.adminDetail(id);
  }
  @RequirePlatformPermissions(READ) @Get("articles/:id/preview") preview(@Param("id", new ParseUUIDPipe()) id: string) {
  ...
  @RequirePlatformPermissions(EDIT) @Patch("articles/:id") update(
    @Param("id", new ParseUUIDPipe()) id: string,
    @Body() body: SaveBlogArticleDto,
  ) {
    return this.blog.update(id, body, this.actor());
  }
  @RequirePlatformPermissions(PUBLISH) @Patch("articles/:id/status") status(
    @Param("id", new ParseUUIDPipe()) id: string,
    @Body() body: ArticleStatusDto,
  ) {
    return this.blog.status(id, body, this.actor());
  }
  @RequirePlatformPermissions(PUBLISH) @Delete("articles/:id") @HttpCode(204)
