...
export class OutsourcedDriverFeeService {
  ...
  public async reverseAccrual(
    ...
    return this.transactions.execute(async (transaction) => {
      ...
      await this.complete(transaction, companyId, idempotencyKey!, operations.accrualReversal, accrualId, response);
      return response;
    });
  }
  ...
  public async accruals(query: OutsourcedDriverFeeAccrualListQueryDto) {
    ...
    `.execute(this.database);
    return { items: result.rows.map(({ totalCount: _total, ...row }) => row), page: page.page, pageSize: page.pageSize, total: Number(result.rows[0]?.totalCount ?? 0) };
  }
  ...
  public async paymentProposal(input: OutsourcedDriverFeePaymentProposalDto) {
    ...
    const { companyId } = this.support.context();
    return this.buildProposal(this.database, companyId, input.driverId, new Decimal(input.amount), input.allocations, false);
  }
  ...
  public async confirmPayment(
    ...
    return this.transactions.execute(async (transaction) => {
      ...
      const reservation = await this.support.reserveIdempotency<FeePaymentResult>(transaction, {
        companyId, idempotencyKey, operation: operations.paymentConfirmation, payload: material,
      });
      if (reservation.replayResponse !== undefined) return reservation.replayResponse;
      if (reservation.replayResourceId !== undefined) return this.paymentResult(transaction, companyId, reservation.replayResourceId);
      const proposal = await this.buildProposal(transaction, companyId, input.driverId, new Decimal(input.amount), input.allocations, true);
      return this.createPayment(transaction, {
        actorId, companyId, correlationId, driverId: input.driverId,
        idempotencyKey: idempotencyKey!, paymentDate: input.paymentDate,
        paymentMethod: "cash", paymentSource: "separate_payment",
        cashVoucherReference: input.cashVoucherReference?.trim() || undefined,
        externalReference: input.externalReference?.trim(), notes: input.notes?.trim(),
        allocations: proposal.allocations.map((line) => ({ accrualId: line.accrualId, amount: line.amount })),
        manualAllocation: input.allocations !== undefined,
      }, operations.paymentConfirmation);
    });
  }

  public async reversePayment(paymentId: string, reason: string, idempotencyKey: string | undefined, correlationId: string) {
    this.support.assertPermission("outsourced_driver_fees.reverse");
    const { actorId, companyId } = this.support.context();
    return this.transactions.execute(async (transaction) => {
      const reservation = await this.support.reserveIdempotency(transaction, {
        companyId, idempotencyKey, operation: operations.paymentReversal, payload: { paymentId, reason: reason.trim() },
      });
      if (reservation.replayResponse !== undefined) return reservation.replayResponse;
      ...
      if (row.status === "reversed") throw new ApplicationException("outsourced_driver_fee_payment_already_reversed", "This Driver fee payment is already reversed", HttpStatus.CONFLICT);
      if (row.paymentSource !== "separate_payment") throw new ApplicationException("outsourced_driver_fee_offset_requires_collection_reversal", "A Driver Collection fee offset must be reversed through Driver Collection reversal", HttpStatus.CONFLICT);
      const response = await this.reversePaymentInTransaction(transaction, companyId, actorId, paymentId, reason, correlationId);
      await this.complete(transaction, companyId, idempotencyKey!, operations.paymentReversal, paymentId, response);
      return response;
    });
  }
  ...
  public async paymentDetail(paymentId: string) {
    ...
    const payment = await sql<Record<string, unknown>>`
      select p.*, d.code as "driverCode", d.name_en as "driverName", d.driver_type as "driverType",
        r.reconciliation_number as "linkedReconciliationNumber",
        coalesce(cu.display_name,a.username) as "paidBy"
      from outsourced_driver_fee_payments p join drivers d on d.id=p.driver_id and d.company_id=p.company_id
      join accounts a on a.id=p.paid_by_account_id and a.company_id=p.company_id
      left join company_users cu on cu.account_id=a.id and cu.company_id=a.company_id
  ...
  public async collectionOffsetProposal(
    ...
    if (requestedAmount.greaterThan(safeMaximum)) {
      throw new ApplicationException("outsourced_driver_fee_offset_above_safe_amount", "The Driver fee offset exceeds the safe collection amount", HttpStatus.CONFLICT);
    }
    const oldest = this.allocate(totalRows, safeMaximum);
    const selected = allocations === undefined
  ...
  public async confirmCollectionOffset(
    database: Database,
    input: {
      readonly actorId: string; readonly allocations?: readonly { accrualId: string; amount: number }[];
      readonly amount: Decimal; readonly companyId: string; readonly correlationId: string;
      readonly driverId: string; readonly idempotencyKey: string; readonly paymentDate: string;
      readonly reconciliationId: string; readonly safeCollectionAmount: Decimal;
    },
  ) {
    if (input.amount.isZero()) return null;
    this.support.assertPermission("outsourced_driver_fees.pay");
    const proposal = await this.collectionOffsetProposal(database, input.companyId, input.driverId, input.safeCollectionAmount, input.amount, input.allocations, true);
    return this.createPayment(database, {
      actorId: input.actorId, companyId: input.companyId, correlationId: input.correlationId,
      driverId: input.driverId, idempotencyKey: `${input.idempotencyKey}:driver-fee`,
      paymentDate: input.paymentDate, paymentMethod: "collection_offset", paymentSource: "driver_collection",
  ...
  public async reverseCollectionOffset(database: Database, companyId: string, actorId: string, reconciliationId: string, reason: string, correlationId: string) {
    ...
    this.support.assertPermission("outsourced_driver_fees.reverse");
    return this.reversePaymentInTransaction(database, companyId, actorId, payment.rows[0].id, reason, correlationId);
  }

  private async runRange(fromDate: string, toDate: string, driverId: string | undefined, source: AccrualSource, preview: boolean, notes: string | undefined, idempotencyKey: string | undefined, correlationId: string) {
    const { actorId, companyId } = this.support.context();
    return this.transactions.execute(async (transaction) => {
      const operation = source === "authorized_backfill" ? operations.backfill : operations.reconciliation;
      const reservation = await this.support.reserveIdempotency(transaction, {
        companyId, idempotencyKey, operation, payload: { fromDate, toDate, driverId, notes, preview },
      });
      if (reservation.replayResponse !== undefined) return reservation.replayResponse;
      if (driverId !== undefined) {
        const driver = await sql`select id from drivers where id=${driverId}::uuid and company_id=${companyId}::uuid`.execute(transaction);
      ...
      for (const order of orders.rows) {
        const outcome = await this.evaluateOrder(transaction, companyId, order.id, source, actorId, correlationId, !preview);
        outcomes.push(outcome);
        if (outcome.outcome === "created" || (preview && outcome.outcome === "eligible")) {
          created += 1; total = total.plus(outcome.amount ?? 0);
        }
      }
      ...
      await this.history.audit(transaction, { action: source === "authorized_backfill" ? "outsourced_driver_fee_backfill_completed" : "outsourced_driver_fee_reconciliation_completed", actorId, after: response, companyId, correlationId, subjectId: companyId, subjectType: "company" });
      await this.complete(transaction, companyId, idempotencyKey!, operation, companyId, response);
      return response;
    });
  }

  private async evaluateOrder(database: Database, companyId: string, orderId: string, source: AccrualSource, actorId: string, correlationId: string, create: boolean) {
    ...
    if (order.driverId === null) return { orderId, orderNumber: order.orderNumber, outcome: "no_assigned_driver" };
    if (order.driverType !== "outsourced") return { orderId, orderNumber: order.orderNumber, outcome: "employee_driver" };
    const existing = await sql<{ status: string }>`select status from outsourced_driver_fee_accruals where company_id=${companyId}::uuid and order_id=${orderId}::uuid`.execute(database);
    if (existing.rows[0] !== undefined) return { orderId, orderNumber: order.orderNumber, outcome: existing.rows[0].status === "reversed" ? "reversed_historical_accrual_prevents_replacement" : "historical_accrual_already_exists" };
    if (await this.legacyRepresented(database, companyId, orderId)) {
      const outcome = { orderId, orderNumber: order.orderNumber, outcome: "represented_by_legacy_commission" };
    ...
    if (rates.rows.length === 0) {
      const outcome = { orderId, orderNumber: order.orderNumber, outcome: "outsourced_driver_fee_rate_missing" };
      await this.auditEligibilityOutcome(database, companyId, actorId, correlationId, outcome);
      return outcome;
    }
    if (rates.rows.length > 1) {
      const outcome = { orderId, orderNumber: order.orderNumber, outcome: "outsourced_driver_fee_rate_ambiguous" };
      await this.auditEligibilityOutcome(database, companyId, actorId, correlationId, outcome);
      return outcome;
    }
    const rate = rates.rows[0]!;
    if (!create) return { amount: rate.amount, orderId, orderNumber: order.orderNumber, outcome: "eligible" };
    ...
    `.execute(database);
    if (inserted.rows[0] === undefined) return { orderId, orderNumber: order.orderNumber, outcome: "historical_accrual_already_exists" };
    const response = { accrualId: inserted.rows[0].id, amount: rate.amount, orderId, orderNumber: order.orderNumber, outcome: "created" };
    await this.history.audit(database, { action: "outsourced_driver_fee_accrual_created", actorId, after: response, companyId, correlationId, subjectId: inserted.rows[0].id, subjectType: "outsourced_driver_fee_accrual" });
    return response;
  }
  ...
  private async buildProposal(database: Database, companyId: string, driverId: string, amount: Decimal, manual: readonly { accrualId: string; amount: number }[] | undefined, lock: boolean) {
    ...
    if (amount.greaterThan(total)) throw new ApplicationException("outsourced_driver_fee_payment_exceeds_outstanding", "The payment exceeds the Driver's current outstanding fees", HttpStatus.CONFLICT);
    const allocations = manual === undefined ? this.allocate(rows, amount) : this.manualAllocate(rows, amount, manual);
    return { allocations, driverId, remainingOutstanding: total.minus(amount).toFixed(2), totalAmount: amount.toFixed(2) };
  }

  private async payableAccruals(database: Database, companyId: string, driverId: string, lock: boolean) {
  ...
  private allocate(rows: readonly { accrualId: string; orderNumber: string; outstanding: string }[], amount: Decimal): AllocationProposal[] {
    ...
    for (const row of rows) {
      ...
      output.push({ accrualId: row.accrualId, amount: allocated.toFixed(2), orderNumber: row.orderNumber, outstandingBefore: outstanding.toFixed(2), remainingOutstanding: outstanding.minus(allocated).toFixed(2) });
      remaining = remaining.minus(allocated);
    }
    return output;
  }

  private manualAllocate(rows: readonly { accrualId: string; orderNumber: string; outstanding: string }[], amount: Decimal, manual: readonly { accrualId: string; amount: number }[]) {
    const byId = new Map(rows.map((row) => [row.accrualId, row]));
    const seen = new Set<string>();
    let total = new Decimal(0);
    const output = manual.map((line) => {
      if (seen.has(line.accrualId)) throw new ApplicationException("outsourced_driver_fee_duplicate_allocation", "The same accrual cannot be allocated twice", HttpStatus.BAD_REQUEST);
      seen.add(line.accrualId);
      const row = byId.get(line.accrualId);
      const value = new Decimal(line.amount);
      if (row === undefined || value.lte(0) || value.gt(row.outstanding)) throw new ApplicationException("outsourced_driver_fee_payment_stale_balance", "A selected accrual is no longer payable for the requested amount", HttpStatus.CONFLICT);
      total = total.plus(value);
      return { accrualId: row.accrualId, amount: value.toFixed(2), orderNumber: row.orderNumber, outstandingBefore: new Decimal(row.outstanding).toFixed(2), remainingOutstanding: new Decimal(row.outstanding).minus(value).toFixed(2) };
    });
    if (!total.equals(amount)) throw new ApplicationException("outsourced_driver_fee_allocation_total_mismatch", "Allocation total must equal the requested payment amount", HttpStatus.BAD_REQUEST);
    return output;
  }

  private async createPayment(database: Database, input: {
    actorId: string; allocations: readonly AllocationInput[]; cashVoucherReference?: string;
    companyId: string; correlationId: string; driverId: string; externalReference?: string;
    idempotencyKey: string; linkedReconciliationId?: string; notes?: string; paymentDate: string;
    paymentMethod: string; paymentSource: string; manualAllocation?: boolean;
  }, idempotencyOperation?: string): Promise<FeePaymentResult> {
    const amount = input.allocations.reduce((sum, row) => sum.plus(row.amount), new Decimal(0));
    const paymentNumber = await this.history.nextReferenceNumber(database, input.companyId, "outsourced_driver_fee_payment", "DFPAY");
    ...
    const created = await sql<{ id: string }>`
      insert into outsourced_driver_fee_payments (
        ...
        linked_driver_reconciliation_id,idempotency_key,request_hash
      ) values (
    ...
    for (const [index, allocation] of input.allocations.entries()) {
      await sql`insert into outsourced_driver_fee_payment_allocations(company_id,payment_id,accrual_id,allocated_amount,allocation_order)
        values(${input.companyId}::uuid,${paymentId}::uuid,${allocation.accrualId}::uuid,${allocation.amount},${index + 1})`.execute(database);
    }
    await this.syncAccruals(database, input.companyId, input.allocations.map((row) => row.accrualId));
    ...
    `.execute(database);
    const response: FeePaymentResult = { allocations: confirmedAllocations.rows, amount: amount.toFixed(2), paymentId, paymentNumber, remainingDriverOutstanding: remaining, status: "confirmed" };
    await this.history.audit(database, { action: input.paymentSource === "driver_collection" ? "outsourced_driver_fee_collection_offset_confirmed" : "outsourced_driver_fee_payment_confirmed", actorId: input.actorId, after: response, companyId: input.companyId, correlationId: input.correlationId, subjectId: paymentId, subjectType: "outsourced_driver_fee_payment" });
    ...
    if (idempotencyOperation !== undefined) await this.complete(database, input.companyId, input.idempotencyKey, idempotencyOperation, paymentId, response);
    return response;
  }

  private async reversePaymentInTransaction(database: Database, companyId: string, actorId: string, paymentId: string, reason: string, correlationId: string) {
    if (reason.trim().length === 0) throw new ApplicationException("outsourced_driver_fee_reversal_reason_required", "A reversal reason is required", HttpStatus.BAD_REQUEST);
    ...
    `.execute(database);
    if (allocations.rows.length === 0) throw new ApplicationException("outsourced_driver_fee_payment_allocation_conflict", "This payment has no active allocations to reverse", HttpStatus.CONFLICT);
    ...
    await sql`select id from outsourced_driver_fee_accruals where company_id=${companyId}::uuid and id in (${sql.join(allocations.rows.map((row) => sql`${row.accrualId}::uuid`))}) order by id for update`.execute(database);
    await sql`update outsourced_driver_fee_payment_allocations set reversed_at=now() where company_id=${companyId}::uuid and payment_id=${paymentId}::uuid and reversed_at is null`.execute(database);
    await sql`update outsourced_driver_fee_payments set status='reversed',reversed_by_account_id=${actorId}::uuid,reversed_at=now(),reversal_reason=${reason.trim()},updated_at=now(),version=version+1 where id=${paymentId}::uuid and company_id=${companyId}::uuid`.execute(database);
    await this.syncAccruals(database, companyId, allocations.rows.map((row) => row.accrualId));
    const response = { paymentId, restoredAccrualIds: allocations.rows.map((row) => row.accrualId), status: "reversed" };
    await this.history.audit(database, { action: "outsourced_driver_fee_payment_reversed", actorId, after: response, companyId, correlationId, subjectId: paymentId, subjectType: "outsourced_driver_fee_payment" });
    return response;
  }
  ...
  private async driverOutstanding(database: Database, companyId: string, driverId: string) {
    const result = await sql<{ total: string }>`select coalesce(sum(outstanding_amount),0)::text as total from outsourced_driver_fee_accruals where company_id=${companyId}::uuid and driver_id=${driverId}::uuid and status in ('accrued','partially_paid')`.execute(database);
    return result.rows[0]!.total;
  }

  private async paymentResult(database: Database, companyId: string, paymentId: string): Promise<FeePaymentResult> {
    const result = await sql<{ amount: string; driverId: string; paymentId: string; paymentNumber: string; status: string }>`select id as "paymentId",payment_number as "paymentNumber",driver_id as "driverId",amount_paid::text as amount,status from outsourced_driver_fee_payments where id=${paymentId}::uuid and company_id=${companyId}::uuid`.execute(database);
    const payment = result.rows[0];
    if (payment === undefined) this.notFound("outsourced_driver_fee_payment_not_found");
    return { allocations: [], amount: payment.amount, paymentId: payment.paymentId, paymentNumber: payment.paymentNumber, remainingDriverOutstanding: await this.driverOutstanding(database, companyId, payment.driverId), status: payment.status };
  }

  private complete(database: Database, companyId: string, idempotencyKey: string, operation: string, resourceId: string, response: unknown) {
    return this.support.completeIdempotency(database, { companyId, idempotencyKey, operation, resourceId, resourceType: "outsourced_driver_fee", responseBody: response });
  }

  private notFound(code: string): never {
    throw new ApplicationException(code, "The requested Outsourced Driver fee record was not found", HttpStatus.NOT_FOUND);
  }
}
