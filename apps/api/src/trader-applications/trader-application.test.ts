...
import { CreateTraderApplicationDto, TraderApplicationListQueryDto } from "./trader-application.dto.js";
...
describe("Trader application list query",()=>{
  it("transforms HTTP query pagination and Delivery Company filters",async()=>{
    const pipe=new ValidationPipe({forbidNonWhitelisted:true,transform:true,whitelist:true});
    const result=await pipe.transform(
      {page:"2",pageSize:"50",requiresDeliveryCompany:"false",status:"reviewing"},
      {type:"query",metatype:TraderApplicationListQueryDto},
    );
    expect(result).toMatchObject({page:2,pageSize:50,requiresDeliveryCompany:false,status:"reviewing"});
  });
});
