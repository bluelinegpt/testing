...
export async function loadHelpHome(locale: Locale) {
  const response = await fetch(apiUrl(`/public/website/help?locale=${locale}`), { cache: 'no-store' });
  if (!response.ok) throw new Error('Help Center could not be loaded.');
  return (await response.json()) as { locale: Locale; direction: 'ltr' | 'rtl'; categories: HelpCategory[]; articles: HelpArticleSummary[] };
}
...
export async function searchHelp(locale: Locale, query: string, audience = 'all', category = '') {
  ...
  const response = await fetch(apiUrl(`/public/website/help/search?${params.toString()}`), { cache: 'no-store' });
...
export async function loadHelpArticle(slug: string, locale: Locale) {
  const response = await fetch(apiUrl(`/public/website/help/articles/${slug}?locale=${locale}`), { cache: 'no-store' });
