...
export function TraderPage() {
  ...
  return <><section className="trader-hero" dir={locale === 'ar' ? 'rtl' : 'ltr'}><div className="trader-hero-inner"><div><p className="eyebrow"><span />{t.eyebrow}</p><h1>{t.title}</h1><p>{t.body}</p><Link className="button button-primary" to="/traders/register" onClick={() => trackEvent('trader_registration_clicked', { page: '/traders', ctaLocation: 'hero', locale })}>{t.cta}</Link></div><div className="trader-channel-cloud"><strong>{t.cloud}</strong>{(locale === 'ar' ? channelsAr : channels).map(x => <span key={x}>{x}</span>)}</div></div></section><section className="section"><div className="section-heading"><p className="eyebrow"><span />{t.sectionEyebrow}</p><h2>{t.sectionTitle}</h2><p>{t.sectionBody}</p></div><div className="trader-benefit-grid">{t.benefits.map(([h, p], i) => <article key={h}><span>0{i + 1}</span><h3>{h}</h3><p>{p}</p>{i === 2 && <b>{t.planned}</b>}</article>)}</div><div className="center-action"><Link className="button button-primary" to="/traders/register" onClick={() => trackEvent('trader_registration_clicked', { page: '/traders', ctaLocation: 'benefits', locale })}>{t.cta}</Link></div></section></>;
}
