...
export function safeEditorHtml(value:string) {
  const html=DOMPurify.sanitize(value,{ ALLOWED_TAGS:["h1","p","div","span","h2","h3","h4","strong","b","em","i","u","s","br","hr","ul","ol","li","blockquote","a","img","figure","figcaption"], ALLOWED_ATTR:["style","dir","href","title","src","alt","width","loading","rel"], ALLOW_DATA_ATTR:false });
  const template=document.createElement("template");template.innerHTML=html;
  template.content.querySelectorAll("h1").forEach(el=>{const heading=document.createElement("h2");heading.append(...Array.from(el.childNodes));el.replaceWith(heading)});
  const styles:Record<string,RegExp>={"font-weight":/^(normal|bold|[1-9]00)$/,"font-style":/^(normal|italic)$/,"text-decoration":/^(none|underline|line-through)$/,"font-family":/^(Arial|Georgia|Verdana|Tahoma|sans-serif|serif)$/i,"font-size":/^(12|14|16|18|20|24|28|32|36|48)px$/,"text-align":/^(left|right|center|justify)$/,width:/^(25|50|75|100)%$/};
...
export function BlogRichEditor({value,onChange,disabled=false}:{value:string;onChange:(html:string)=>void;disabled?:boolean}) {
