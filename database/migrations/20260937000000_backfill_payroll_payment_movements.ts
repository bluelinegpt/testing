...
export async function up(database: Kysely<MigrationDatabase>): Promise<void> {
  const missing = await sql<{
    ...
    select p.paid_by_account_id as "actorId",p.total_amount::text as amount,
      ...
      join accounting_events e on e.company_id=p.company_id
       ...
       and e.source_entity_id=p.id and e.event_type='employee_payroll_paid'
     where p.status='confirmed' and p.payment_method='cash'
       and p.company_cash_account_id is not null
       and not exists (
         select 1 from cash_bank_movements m
          where m.company_id=p.company_id
            and m.idempotency_identity='payroll_payment:'||p.id::text
       )
  ...
  for (const payment of missing.rows) {
    ...
    await sql`
      insert into cash_bank_movements(
        ...
        0,'cash',${payment.paymentNumber},${`Employee Payroll payment ${payment.paymentNumber}`},
        'confirmed',${payment.paymentId},${`payroll_payment:${payment.paymentId}`},
